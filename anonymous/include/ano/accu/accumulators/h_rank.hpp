    ...
    namespace accumulators
    {
      ...
      private:
        void m_update()
        {
          ...
          if (m_cumsum <= P)
          {
            ano_assertion(m_current < (int)(m_hist.size() - 1));
          ...
          if (EQ < Q)
          {
            ano_assertion(m_current > 0);
            m_cumsum -= m_hist[m_current];
            while (m_hist[--m_current] == 0)
              ;
          }

          ano_assertion(m_cumsum > P);
          ano_assertion((m_count - m_cumsum + m_hist[m_current]) >= Q);
        }
