...
#include <ano/accu/concept/accumulator.hpp>
#include <ano/accu/feature.hpp>
...
namespace ano
{
  ...
  namespace accu
  {
    ...
    namespace internal
    {
      ...
      {
        typedef
            typename mpl::remove_if<Depends, mpl::has_key<FeatureSet, ph::_1>, mpl_set_inserter<mpl::set<>>>::type type;
      };
    ...
    struct composite_accumulator_base : Accumulator<E>
    {
      ...
      {
        boost::fusion::for_each(m_accus, internal::accu_take<Other>(ano::exact(other)));
      }
      ...
      const E& to_result() const { return ano::exact(*this); }
                                   ...
                                   features::composite_feature<typename features::depends<Feature>::type>>
    {
      ...
      ResultType to_result() const { return extract(ano::exact(*this), Feature()); }
    };
  } // namespace accu
} // namespace ano
