...
namespace ano::bp
{
  ...
  /// \endrst
  template <typename T>
  void copy(const T* __restrict src, T* dst, int width, int height, std::ptrdiff_t src_stride,
                   ...
                   std::ptrdiff_t dst_stride)
  {
    ...
    for (int y = 0; y < h; ++y)
      std::copy_n(ano::bp::ptr_offset(src, y * src_stride), w, ano::bp::ptr_offset(dst, y * dst_stride));
  }
