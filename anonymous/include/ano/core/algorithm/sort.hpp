...
#include <ano/core/value/indexer.hpp>
...
namespace ano
{
  ...
  namespace impl
  {
    ...
    {
      // Comute histogram
      ano::for_each(input, [histogram, proj](image_value_t<I> v) { ++histogram[proj(v)]; });
    ...
    {
      ano_entering("ano::sort (counting sort)");
    ...
    {
      ano_entering("ano::sort (quicksort)");
  ...
  {
    static_assert(ano::is_a<InputImage, ano::details::Image>(), "Input is not an image.");
      ...
      std::vector<image_index_t<InputImage>> sort_indexes(InputImage input, Compare cmp)
  {
    ...
    ano::sort_indexes(std::move(input), out, cmp);
    return out;
  }
      ...
      std::vector<image_point_t<InputImage>> sort_points(InputImage input, Compare cmp)
  {
    ...
    ano::sort_points(std::move(input), out, cmp);
    return out;
  }
