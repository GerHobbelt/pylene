...
namespace ano
{
  ...
  void transform(InputImage in, OutputImage out, UnaryFunction f)
  {
    ...
    ano_entering("ano::transform");
    ano_precondition(in.domain() == out.domain());

    auto&& ivals = in.values();
    auto&& ovals = out.values();
    for (auto [r1, r2] : ano::ranges::view::zip(ranges::rows(ivals), ranges::rows(ovals)))
  ...
  requires ::ranges::invocable<BinaryFunction, image_reference_t<InputImage1>, image_reference_t<InputImage2>>
      ...
      transform(InputImage1 in1, InputImage2 in2, BinaryFunction f)
  {
    static_assert(ano::is_a<InputImage1, ano::details::Image>());
    static_assert(ano::is_a<InputImage2, ano::details::Image>());
  ...
  void transform(InputImage1 in1, InputImage2 in2, OutputImage out, BinaryFunction f)
  {
    ...
    // ano_precondition(in2.domain() == out.domain());
    // ano_precondition(in1.domain() == out.domain());

    auto&& ivals1 = in1.values();
    auto&& ivals2 = in2.values();
    auto&& ovals  = out.values();
    for (auto [r1, r2, r3] : ano::ranges::view::zip(ranges::rows(ivals1), ranges::rows(ivals2), ranges::rows(ovals)))
    ...
    namespace details
    {
      ...
      class TransformParallel : public ParallelCanvas2d
      {
        ...
        void ExecuteTile(ano::box2d b) const final
        {
          ...
          ano::transform(subimage_in, subimage_out, _fun);
        }
      ...
      class Transform_BinaryFunc_Parallel : public ParallelCanvas2d
      {
        ...
        ano::box2d GetDomain() const final { return _in1.domain(); }
        ...
        void ExecuteTile(ano::box2d b) const final
        {
          ...
          auto subimage_out = _out.clip(b);
          ano::transform(subimage_in1, subimage_in2, subimage_out, _fun);
        }
