...
#include <ano/core/range/mdindex.hpp>
...
namespace ano
{
  ...
  public:
    ...
    {
      return ano::box2d{y(), x(), width(), height()};
    }
  ...
  namespace impl
  {
    ...
    struct _bstorage : ano::ranges::mdindex_facade<D, T, ndpoint<D, T>, _bstorage<D, T>>
    {
    private:
    ...
    public:
      ...
      using base = ano::ranges::mdindex_facade<D, T, point_type, _bstorage<D, T>>;
    ...
    template <int D, class T>
    struct _bref
      : ano::ranges::mdindex_facade<D, std::remove_const_t<T>, ndpoint<D, std::remove_const_t<T>>, _bref<D, T>>
    {
      using base =
          ano::ranges::mdindex_facade<D, std::remove_const_t<T>, ndpoint<D, std::remove_const_t<T>>, _bref<D, T>>;
...
namespace concepts
{
  ...
  struct basic_common_reference<ano::_box<UImpl>, ano::_box<VImpl>, TQual, UQual>
  {
    static_assert((UImpl::ndim == -1) || (VImpl::ndim == -1) || (UImpl::ndim == UImpl::ndim),
                  "Incompatible number of dimensions.");
    using type = ano::ndbox<(UImpl::ndim == VImpl::ndim) ? UImpl::ndim : -1>;
  };
} // namespace concepts
...
namespace ranges
{
  ...
  struct range_cardinality<ano::_box<Impl>, void> : std::integral_constant<cardinality, finite>
  {
  };
} // namespace ranges
