...
// This concept check makes an ICE with MSVC
  ...
  namespace details
  {
    ...
    template <class Impl>
    struct is_domain_row_contiguous<ano::_box<Impl>> : std::true_type
    {
    };
  } // namespace details
  ...
  void IncrementalLocalAlgorithm<SE, I, J, Self>::Execute()
  {
    ...
    {
      ano::trace::warn("[Performance] The accumulator is not incremental.");
      this->LocalAlgorithm<SE, I, J, Self>::Execute();
      return;
    }


    ano_entering("LocalAlgorithm::Execute (Incremental)");
