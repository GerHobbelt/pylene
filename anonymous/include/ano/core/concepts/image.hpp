...
#include <ano/core/concepts/extension.hpp>
...
#include <ano/core/private/traits/image.hpp>
...
#include <ano/core/concepts/object.hpp> // ano::is_a
...
namespace ano::concepts
{
  ...
  concept Image =
    ...
    std::is_base_of_v<ano::details::Image<I>, I> &&
    ...
    requires {
      ...
      typename image_ch_value_t<I, ano::archetypes::Value>;
      // traits
    ...
    requires(I ima, const I cima, image_domain_t<I> d, image_point_t<I> p) {
      { cima.template ch_value<ano::archetypes::Value>() }
      -> ::concepts::convertible_to<image_ch_value_t<I, ano::archetypes::Value>>;
      { cima.concretize() } -> ::concepts::convertible_to<image_concrete_t<I>>;
      { cima.domain() }     -> ::concepts::convertible_to<image_domain_t<I>>;
      { ima.pixels() }  -> ano::ranges::mdrange;
      { ima.values() }  -> ano::ranges::mdrange;
      requires ::concepts::convertible_to<ano::ranges::mdrange_value_t<decltype(ima.pixels())>, image_pixel_t<I>>;
      requires ::concepts::convertible_to<ano::ranges::mdrange_value_t<decltype(ima.values())>, image_value_t<I>>;
    };
  ...
  namespace detail
  {
    ...
    concept WritableImage =
      ...
      requires(I ima) {
      { ima.values() }  -> ano::ranges::output_mdrange<image_value_t<I>>;
        // Check Writability of each pixel of the range
        requires OutputPixel<
                   std::common_type_t<
                     ano::ranges::mdrange_value_t<decltype(ima.pixels())>,
                     image_pixel_t<I>>>;
      };
  ...
  concept BidirectionalImage =
    ...
    requires (I ima) {
    { ima.pixels() }  -> ano::ranges::reversible_mdrange;
    { ima.values() }  -> ano::ranges::reversible_mdrange;
  };
  ...
  concept WithExtensionImage =
    ...
    not ::concepts::same_as<ano::extension::none_extension_tag, image_extension_category_t<I>> &&
    requires (I ima, image_point_t<I> p) {
