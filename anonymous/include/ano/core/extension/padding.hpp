...
#include <ano/core/range/mdspan_fwd.hpp>
...
namespace ano
{
  ...
  ///
  /// Given the input `[1 2 3]` defined on domain [0-3] and the output defined on domain [-2, 7] and value = `9`. The
  /// result in output is:
  ///
  /// PAD_ZERO
  ///   pad each dimension with value zero `[0 0 1 2 3 0 0 0 0]`
  ...
  /// \param value The value used when mode=PAD_CONSTANT
  template <class T, int dim>
  void copy_pad(ranges::mdspan<T, dim> input, ranges::mdspan<T, dim> out, ano::ndbox<dim> roi, e_padding_mode mode,
                T value = {});
  ...
  /// \brief Transpose a region from a 2D image or a 2D-span and pad the missing values using one of the padding
  /// strategy
  ///
  /// Given the input `[1 2 3]` defined on domain [0-3] and the output defined on domain [-2, 7] and value = `9`. The
  /// result in output is:
  ...
  void pad(const ranges::mdspan<T, dim>& sp, e_padding_mode mode, const int borders[][2], T value)
  {
    ...
    T* vptr = (mode == ano::PAD_ZERO) ? nullptr : &value;
                ...
                T value)
  {
    ...
    T* vptr = (mode == ano::PAD_ZERO) ? nullptr : &value;
    impl::copy_pad(&_padder, dim, (std::byte*)out.data(), (std::byte*)input.data(), roi, sizes, ostrides, istrides,
                   mode, vptr);
  }
  ...
  void copy_pad(const ndimage<T, dim>& input, ndimage<T, dim>& output, e_padding_mode mode, T value)
  {
    ...
    std::ptrdiff_t  ostrides[dim];
    int             sizes[dim];
    ano::ndbox<dim> roi = output.domain();
    ...
    T* vptr = (mode == ano::PAD_ZERO) ? nullptr : &value;
    impl::copy_pad(&_padder, dim, (std::byte*)output.buffer(), (std::byte*)input.buffer(), roi, sizes, ostrides,
                   istrides, mode, vptr);
  }
  ...
  namespace impl
  {
    ...
    void pad_generic(I image, e_padding_mode mode, const int borders[][2], T value, ano::ndbox<dim> domain)
    {
      ndbox<dim> inner;
      ...
      ano_foreach (auto p, domain)
        if (!inner.has(p))
          ...
          case PAD_CONSTANT:
            ...
            break;
          case PAD_WRAP:
            image(p) = image(inner.periodize(p));
            break;
          case PAD_MIRROR:
            image(p) = image(inner.mirror(p));
            break;
          case PAD_REPLICATE:
            image(p) = image(inner.clamp(p));
            break;
          }
    }
    ...
    void copy_pad_generic(I in, J out, e_padding_mode mode, T value, ano::ndbox<dim> domain)
    {
    ...
    void transpose_pad_generic(I in, J out, e_padding_mode mode, T value, ano::box2d domain)
    {
      auto inner = in.domain();

      ano::box2d dom_T(domain.y(), domain.x(), domain.height(), domain.width());

      ano_foreach (auto p, dom_T)
      {
        point2d q = {p.y(), p.x()};
  ...
  void pad(I image, e_padding_mode mode, const int borders[][2], image_value_t<I> value)
  {
    static_assert(std::is_convertible_v<image_domain_t<I>, ano::Box>);
  ...
  {
    static_assert(std::is_convertible_v<image_domain_t<I>, ano::Box>);
    static_assert(std::is_convertible_v<image_domain_t<J>, ano::Box>);
