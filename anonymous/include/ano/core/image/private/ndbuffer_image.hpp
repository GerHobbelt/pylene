...
#include <ano/core/image/private/ndbuffer_image_pixel.hpp>
#include <ano/core/image/private/ndimage_extension.hpp>
#include <ano/core/range/mdspan.hpp>
...
namespace ano
{
  ...
  template <class T, int N>
  class __ndbuffer_image : public __ndbuffer_image<void, -1>, public ano::details::Image<__ndbuffer_image<T, N>>
  {
    using base = __ndbuffer_image<void, -1>;
    using Impl = ano::details::ndbuffer_image_impl<T, N>;
  ...
  public:
    ...
    using extension_category = ano::extension::border_extension_tag;
    /// \}
    ...
    /// Value access
    /// \{
    value_set<T>    get_value_set() const noexcept;
    ...
    template <class E>
    static __ndbuffer_image<T, N> from(const ano::ndimage_base<T, N, E>& other);
    /// \}
  ...
  template <class T, int N>
  template <class E>
  inline __ndbuffer_image<T, N> __ndbuffer_image<T, N>::from(const ano::ndimage_base<T, N, E>& other)
  {
  ...
  template <class T, int N>
  value_set<T> __ndbuffer_image<T, N>::get_value_set() const noexcept
  {
    return {};
  }
  ...
  template <class T, int N>
  ano::internal::ndimage_extension<T, N> __ndbuffer_image<T, N>::extension() const noexcept
  {
    ...
    return ano::internal::ndimage_extension<T, N>{(char*)ptr, strides, dims, this->border()};
  }
