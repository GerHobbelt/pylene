...
namespace ano
{
  ...
  class zip_view : public ano::details::Image<zip_view<Images...>>
  {
  ...
  public:
    ...
    using extension_category = ano::extension::none_extension_tag; // Preservative behavior
  ...
  public:
    ...
    auto pixels()
    {
      auto g_pixels = [](auto&&... images) {
        return ranges::view::zip_with(ano::details::make_object<pixel_type>{}, images.pixels()...);
      };
    ...
    {
      ano_precondition(all_domain_has(p));
      return this->at(p);
    }
  ...
  namespace view
  {
    ...
    {
      static_assert(std::conjunction_v<is_a<Images, ano::details::Image>...>, "All zip arguments must be images.");
