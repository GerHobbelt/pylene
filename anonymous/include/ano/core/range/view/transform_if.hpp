...
#include <ano/core/range/range.hpp>
...
namespace ano::ranges
{
  ...
  namespace details
  {
    ...
    struct transform_if_mdcursor : mdcursor_facade<transform_if_mdcursor<F, Pred, Cur...>>
    {
      ...
      struct cursor
      {
        decltype(auto) read() const { return std::apply(m_map_fn, m_cursors); }
      ...
      {
        auto c = std::apply([](auto&&... cur) { return std::make_tuple(::ranges::range_access::begin_cursor(cur)...); },
                            m_cursors);
  ...
  namespace view
  {
    ...
    {
      static_assert(::ranges::invocable<F, ano::ranges::mdrange_reference_t<Rng>...>);
      static_assert(::ranges::predicate<Pred, ano::ranges::mdrange_value_t<Rng>...>);
