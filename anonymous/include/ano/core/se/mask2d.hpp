...
#include <ano/core/private/weighted_point.hpp>
...
namespace ano::se
{
  ...
  public:
    ...
    [[gnu::pure]] ::ranges::span<const ano::point2d> offsets() const;
    ...
    ano::box2d compute_input_region(ano::box2d roi) const noexcept;

    /// \brief Return the output region (the valid inner region)
    ///
    /// \pre ``roi.includes(this->se.compute_output_region(roi)``
    ano::box2d compute_output_region(ano::box2d roi) const noexcept;


  private:
    std::vector<ano::point2d> m_points;
    int                       m_radial_extent;
  };
  ...
  {
    using wpoint = ano::weighted_point<ano::point2d, W>;
