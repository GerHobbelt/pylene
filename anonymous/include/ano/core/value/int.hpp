...
#include <ano/core/value/index.hpp>
...
{
  template <unsigned nbits>
  struct is_arithmetic<ano::UInt<nbits>> : std::true_type
  {
  };

  template <unsigned nbits>
  struct is_arithmetic<ano::Int<nbits>> : std::true_type
  {
  };

  template <unsigned nbits>
  struct is_signed<ano::Int<nbits>> : std::true_type
  {
  };

  template <unsigned nbits>
  struct is_unsigned<ano::UInt<nbits>> : std::true_type
  {
  };

  template <unsigned nbits>
  struct make_signed<ano::UInt<nbits>>
  {
    typedef ano::Int<nbits> type;
  };

  template <unsigned nbits>
  struct make_signed<ano::Int<nbits>>
  {
    typedef ano::Int<nbits> type;
  };

  template <unsigned nbits>
  struct make_unsigned<ano::UInt<nbits>>
  {
    typedef ano::UInt<nbits> type;
  };

  template <unsigned nbits>
  struct make_unsigned<ano::Int<nbits>>
  {
    typedef ano::UInt<nbits> type;
  };
} // namespace std
...
namespace std
{
  ...
  struct common_type<ano::UInt<n1>, ano::UInt<n2>>
  {
    typedef ano::UInt<(n1 > n2 ? n1 : n2)> type;
  };

  template <unsigned n, typename V>
  struct common_type<ano::UInt<n>, V> : common_type<typename boost::uint_t<n>::least, V>
  {
  };

  template <unsigned n, typename V>
  struct common_type<V, ano::UInt<n>> : common_type<V, typename boost::uint_t<n>::least>
  {
  };

  template <unsigned n1, unsigned n2>
  struct common_type<ano::Int<n1>, ano::UInt<n2>>
  {
    typedef ano::Int<(n1 > n2 ? n1 : n2)> type;
  };

  template <unsigned n, typename V>
  struct common_type<ano::Int<n>, V> : common_type<typename boost::int_t<n>::least, V>
  {
  };

  template <unsigned n, typename V>
  struct common_type<V, ano::Int<n>> : common_type<V, typename boost::int_t<n>::least>
  {
  };
} // namespace std
