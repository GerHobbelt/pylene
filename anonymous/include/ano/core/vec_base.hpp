...
#include <ano/core/literal/vectorial.hpp>
...
#define VEC_BASE_ENABLE_IF(Trait, R) typename std::enable_if<vec_base_traits<tag>::Trait, R>::type
...
#define VEC_BASE_GEN_EW_OP(Trait, Op)                                                                                  \
  template <typename U, typename = typename std::enable_if<vec_base_traits<tag>::Trait, U>::type>                      \
...
#define VEC_BASE_GEN_REL_ALL(Trait, Op)                                                                                \
  template <typename U>                                                                                                \
  typename std::enable_if<vec_base_traits<tag>::Trait, bool>::type operator Op(const vec_base<U, dim, tag>& o) const   \
  {                                                                                                                    \
    for (unsigned i = 0; i < dim; ++i)                                                                                 \
      if (!(v_[i] Op o[i]))                                                                                            \
        return false;                                                                                                  \
...
#pragma warning(pop)
    ...
    inline constexpr T& get(ano::internal::vec_base<T, dim, tag>& vec)
    {
      return vec.v_[N];
    }

    template <size_t N, class T, unsigned dim, typename tag>
    inline constexpr const T& get(const ano::internal::vec_base<T, dim, tag>& vec)
    {
      return vec.v_[N];
    }
...
namespace std
{
  ...
  template <typename T, unsigned dim, typename tag>
  struct is_signed<ano::internal::vec_base<T, dim, tag>> : is_signed<T>
  {
  };

  template <typename T, unsigned dim, typename tag>
  struct is_unsigned<ano::internal::vec_base<T, dim, tag>> : is_unsigned<T>
  {
  };

  template <typename T, unsigned dim, typename tag>
  struct make_signed<ano::internal::vec_base<T, dim, tag>>
  {
    typedef ano::internal::vec_base<typename make_signed<T>::type, dim, tag> type;
  };

  template <typename T, unsigned dim, typename tag>
  struct make_unsigned<ano::internal::vec_base<T, dim, tag>>
  {
    typedef ano::internal::vec_base<typename make_unsigned<T>::type, dim, tag> type;
  };

  template <typename T1, typename T2, unsigned dim, typename Tag>
  struct common_type<ano::internal::vec_base<T1, dim, Tag>, ano::internal::vec_base<T2, dim, Tag>>
  {
    typedef ano::internal::vec_base<typename std::common_type<T1, T2>::type, dim, Tag> type;
  };
} // namespace std
...
{
  template <typename T, unsigned dim, typename tag>
  struct promote<ano::internal::vec_base<T, dim, tag>>
  {
    typedef ano::internal::vec_base<typename boost::promote<T>::type, dim, tag> type;
  };
} // namespace boost
