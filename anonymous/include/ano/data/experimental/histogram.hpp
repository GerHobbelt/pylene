...
namespace ano::data::experimental
{

  template <class I, class OutputRng>
  [[gnu::noinline]] void histogram(I image, OutputRng histogram);


  template <class I>
  [[gnu::noinline]] std::vector<std::size_t> histogram(I image);
  ...
  namespace impl
  {
    ...
    void histogram(I&& image, OutputRng hist)
    {
      ...
      ::ranges::fill(hist, 0);
      ano::for_each(std::forward<I>(image), [&hist](int x) { hist[x]++; });
    }
  } // namespace impl
