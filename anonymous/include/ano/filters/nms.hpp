...
namespace ano::filters
{
  ...
  {
    auto r   = ano::se::rect2d{min_xdistance, min_ydistance};
    auto dil = ano::morpho::dilation(input, r);
    ...
    {
      minv = 0.5f * ano::accumulate(input, accu::accumulators::max<T>());
    }
    ...
    {
      auto zz = ano::ranges::view::zip(input.pixels(), dil.pixels());
      ano_foreach ((auto [px_v, px_m]), zz)
      {
