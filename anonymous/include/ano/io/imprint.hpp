...
namespace ano::io
{
  ...
  namespace internal
  {
    ...
    /// \{
    template <class I>
    void imprint(I&& image, const ano::box2d* roi)
    {
      imprint2d(std::forward<I>(image), *roi);
    }

    template <class I>
    void imprint(I&& image, const ano::box3d* roi)
    {
    ...
    {
      ano_foreach (auto px, image.pixels())
        fmt::print("{{{},{}}}\n", px.point(), px.val());
    }
    /// \}
  } // namespace internal
  ...
  void imprint(I&& image, [[maybe_unused]] bool print_border)
  {
    ...
    static_assert(ano::is_a<U, ano::details::Image>());
    ...
    if constexpr (std::is_convertible_v<image_extension_category_t<std::remove_reference_t<I>>,
                                        ano::extension::border_extension_tag>)
      if (print_border)
