...
namespace ano::labeling::experimental
{
  ...
  namespace impl
  {
    ...
    [[gnu::noinline]] int blobs_no_boundcheck(I& ima, N nbh, O& out)
    {
      ...
      ano_entering("ano::labeling::blobs (no_boundcheck)");
      ...
      ano::extension::fill(out, LBL_MAX);

      ano_foreach_new (auto p, ima.domain())
      {
        if (!ima(p) || out(p) > 0)
          continue;
    ...
    [[gnu::noinline]] int blobs_boundcheck(I& ima, N nbh, O& out)
    {
      ...
      ano_entering("ano::labeling::blobs (boundcheck)");
