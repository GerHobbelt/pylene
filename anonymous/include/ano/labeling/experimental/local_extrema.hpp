...
namespace ano::labeling::experimental
{
  ...
  namespace impl
  {
    ...
    [[gnu::noinline]] int local_minima(I input, N nbh, O output, Compare cmp)
    {
      ...
      using ano::morpho::experimental::canvas::impl::zfindroot;
      ...
      {
        // Forward scan
        ano_foreach_new (auto p, input.domain())
        {
          auto fp = input(p);
      ...
      // Labelisation
      {
        ano_foreach_new (auto pxOut, output.new_pixels())
        {
          if (pxOut.val())
          {
            auto p = pxOut.point();
      ...
      ano_assertion((n_label <= std::numeric_limits<Label_t>::max()) &&
  ...
  {
    ano_entering("ano::labeling::local_minima");
