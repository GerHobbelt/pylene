...
namespace ano::morpho
{
  ...
  public:
    ...
    ///
    /// \param node_map Image point -> node_id mapping
    ano::image2d<double> saliency(ano::image2d<int> node_map, ::ranges::span<double> values) const;
  ...
  {
    ano_foreach (auto& id, node_map.values())
    {
      if (id > 0 && !pred(id))
        id = this->parent[id];
    }
  ...
  {
    ano_entering("ano::morpho::component_tree::filter");
  ...
  {
    ano_entering("ano::morpho::component_tree::compute_attribute_on_points");

    static_assert(ano::is_a<I, ano::details::Image>());
    static_assert(ano::is_a<Accu, ano::AccumulatorLike>());
  ...
  {
    ano_entering("ano::morpho::component_tree::compute_attribute_on_values");
    ...
    // Accumulate for each point
    auto zz = ano::view::zip(node_map, input);
    ano_foreach ((auto [node_id, val]), zz.values())
  ...
  {
    ano_entering("ano::morpho::component_tree::compute_attribute_on_pixels");
    ...
    // Accumulate for each point
    ano_foreach (auto px, values.pixels())
  ...
  {
    ano_entering("ano::morpho::component_tree::horizontal_cut_from_levels");
    ...
    auto out = ano::clone(nodemap);
    ano_foreach (auto px, out.pixels())
  ...
  {
    ano_entering("ano::morpho::component_tree::reconstruction");

    image_ch_value_t<I, V> out = imchvalue<V>(node_map);

    auto zz = ano::view::zip(node_map, out);
    ano_foreach ((auto&& [node_id, val]), zz.values())
      val = values[node_id];
