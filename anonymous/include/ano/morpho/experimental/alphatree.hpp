...
#include <ano/morpho/experimental/component_tree.hpp>
...
namespace ano::morpho::experimental
{
  ...
  alphatree(I input, N nbh, F distance)
  {
    ...
    // 4. Compute the hierarchy
    {
      ...
      node_count = internal::alphatree_compute_hierarchy(
          edges.data() + edges_null_count, edges.size() - edges_null_count, node_map, node_count, par, levels);
    }
