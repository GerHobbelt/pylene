...
#include <ano/core/algorithm/sort.hpp>
#include <ano/core/concepts/image.hpp>
#include <ano/core/extension/border_management.hpp>

#include <ano/core/trace.hpp>
#include <ano/core/utils/dontcare.hpp> // detail::ignore_t

#include <range/v3/view/reverse.hpp>

namespace ano::morpho::experimental::canvas
{
  ...
  struct unionfind_visitor_base
  {
    ...
    ignore_t on_union([[maybe_unused]] ignore_t p, [[maybe_unused]] ignore_t rp, [[maybe_unused]] ignore_t q,
                      [[maybe_unused]] ignore_t rq)
    {
      return {};
    }
  ...
  namespace impl
  {
    ...
    {
      P                                                   par;
      ano::morpho::experimental::canvas::detail::ufstatus status;
    };
    ...
    {
      using ano::morpho::experimental::canvas::detail::ufstatus;

      // Forward pass
      {
        ano_entering("Union-find forward pass");
      ...
      // Backward pass
      {
        ano_entering("Union-find Backward pass");
        for (auto p : S)
        {
          auto q = par(p);
          auto r = par(q);
    ...
    [[gnu::noinline]] void union_find_init_par(I par) noexcept
    {
      ano_foreach_new (auto px, par.new_pixels())
        px.val() = px.point();
    }
  ...
  void union_find(I input, N nbh, UFViz viz, Compare cmp)
  {
    ano_entering("ano::morpho::canvas::union_find");
    ...
    // Status image
    auto bm     = ano::extension::bm::fill(ufstatus::NONE);
