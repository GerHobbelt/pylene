...
namespace ano::morpho::experimental
{
  ...
  namespace impl
  {
    ...
    struct extinction_tranform_ufind_visitor
    {
      ...
      void on_finish(P p, dontcare_t) noexcept { m_output(p) = std::abs(m_ima(p) - m_output(p)); }
    ...
    {
      ano_entering("ano::morpho::maxima_extinction_transform");
      image_concrete_t<I> output = imconcretize(input);

      extinction_tranform_ufind_visitor viz(input, output, cmp);
      ano::morpho::experimental::canvas::union_find(input, nbh, viz, cmp);
      return output;
    }
