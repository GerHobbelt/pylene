...
namespace ano::morpho::experimental
{
  ...
  void opening(InputImage&& image, const ano::StructuringElement<SE>& se, BorderManager bm, OutputImage&& out);
  ...
  opening(InputImage&& image, const ano::StructuringElement<SE>& se, BorderManager bm,
  ...
  opening(InputImage&& image, const ano::StructuringElement<SE>& se, OutputImage&& out);
  ...
  void opening(InputImage&& image, const ano::StructuringElement<SE>& se, BorderManager bm, OutputImage&& out)
  {
    ano_entering("ano::morpho::experimental::opening");
    ...
    auto tmp = ano::morpho::experimental::erosion(image, se, bm);
    ano::morpho::experimental::dilation(tmp, se, bm, out); // FIXME: we should use the symmetric here
  }
  ...
  {
    ano_entering("ano::morpho::experimental::opening");
    auto tmp = ano::morpho::experimental::erosion(image, se);
    ano::morpho::experimental::dilation(tmp, se, out); // FIXME: we should use the symmetric here
  }
                                                                ...
                                                                const ano::StructuringElement<SE>& se)
  {
    ...
    ano::morpho::experimental::opening(image, se, out);
    return out;
  }
  ...
  {
    auto out = imconcretize(image).build();
    ano::morpho::experimental::opening(image, se, bm, out);
    return out;
  }
