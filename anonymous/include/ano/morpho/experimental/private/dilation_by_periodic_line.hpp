...
#include <ano/morpho/experimental/private/dilation_vertical_block2d.hpp>
#include <ano/morpho/experimental/private/running_max_1d.hpp>
...
namespace ano::morpho::details
{
  ...
  [[gnu::noinline]] void dilation_by_periodic_line(I in, J& out, const ano::se::periodic_line2d& line,
                                                   BinaryFunction sup, ano::box2d roi);
  ...
  template <class I>
  [[gnu::noinline]] void copy_to_periodic_line(I f, ano::point2d origin, ano::point2d direction, std::size_t n,
  ...
  // fixme: could be optimized for indexable images
  template <class J>
  [[gnu::noinline]] void copy_from_periodic_line(const image_value_t<J>* __restrict buffer, ano::point2d origin,
                                                 ano::point2d direction, std::size_t n, J output)
  {
    auto p = origin;
    for (std::size_t i = 0; i < n; ++i, p += direction)
      output(p) = buffer[i];
  ...
  template <class I, class J, class BinaryFunction>
  void dilation_by_periodic_line_generic(I in, J& out, const ano::se::periodic_line2d& line, BinaryFunction sup,
                                         ano::box2d roi)
  {
    ...
    auto fun = [&, k](ano::point2d origin, ano::point2d dir, std::size_t n) {
      copy_to_periodic_line(in, origin - k * period, dir, n + 2 * k, p.get());
      // copy_to_periodic_line(in, origin, dir, n, p.get() + k);
      ano::morpho::experimental::details::running_max_1d(p.get() + k, g.get() + k, h.get() + k, (int)n, k, sup);
      copy_from_periodic_line(p.get() + k, origin, dir, n, out);
    };

    ano::canvas::details::traverse_along_direction(roi, period, fun);
  }

  // Dispatch to generic version
  template <class I, class J, class BinaryFunction>
  void dilation_by_periodic_line(I in, J& out, const ano::se::periodic_line2d& line, BinaryFunction sup, ano::box2d roi)
  {
  ...
  dilation_by_periodic_line(I in, ano::image2d<T>& out, const ano::se::periodic_line2d& line, BinaryFunction sup,
                            ano::box2d roi)
  {
    int                   k      = line.repetition();
    ...
    if (line.is_vertical())
    {
      ano_entering("Running specialization for vertical dilation over 2d buffer with arithmetic types");

      ano::morpho::details::running_max_2d<T>(in, out, sup, roi, k, /* use_extension = */ true,
                                              /* vertical = */ true);
    }
    else if (line.is_horizontal())
    {
      ano_entering("Running specialization for horizontal dilation over 2d buffer with arithmetic types");
