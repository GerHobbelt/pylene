...
namespace ano::morpho::details
{
  ...
  void running_max_2d(I input, J& output, BinaryFunction sup, ano::box2d roi, int k, bool use_extension, bool vertical);
  ...
  public:
    ...
    void running_max_block2d(std::byte* f, std::byte* g, std::byte* h, std::ptrdiff_t f_byte_stride,
                             std::ptrdiff_t g_byte_stride, std::ptrdiff_t h_byte_stride, ano::box2d roi, int k,
                             bool use_extension);

    // Apply the running max algorithm over a roi using tiling
    void execute(ano::box2d roi, int k, bool use_extension, bool vertical = true);
