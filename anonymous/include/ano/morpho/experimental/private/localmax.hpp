...
#include <ano/morpho/experimental/private/dilation_by_periodic_line.hpp>
...
namespace ano::morpho::experimental::details
{
  ...
  namespace impl
  {
    ...
    void localmax(I in, J& out, ValueSet& vs, const ano::se::periodic_line2d& se, const ano::box2d& roi)
    {
      ano::trace::warn("[Performance] Running the specialization with perodic lines.");
      ano::morpho::details::dilation_by_periodic_line(in, out, se, vs.sup, roi);
    }

    // Specialized - Periodic line with box2d domain
    template <class I, class ValueSet>
    void localmax_inplace(I& inout, ValueSet& vs, const ano::se::periodic_line2d& se, const ano::box2d& roi)
    {
      ano::trace::warn("[Performance] Running the specialization with perodic lines.");
      ano::morpho::details::dilation_by_periodic_line(inout, inout, se, vs.sup, roi);
    }

    // Generic - Regular SE over a domain
    template <class I, class J, class ValueSet, class SE, class D>
    void localmax(I in, J& out, ValueSet& vs, const ano::details::StructuringElement<SE>& se, const D&)
    {
      if constexpr (SE::incremental::value && ValueSet::has_incremental_sup::value)
      {
        ano::canvas::LocalAccumulation alg = {vs.accu_incremental_sup, static_cast<const SE&>(se), in, out};
        alg.Execute();
      }
      else
      {
        ano::canvas::LocalAccumulation alg = {vs.accu_sup, static_cast<const SE&>(se), in, out};
        alg.Execute();
      }
    }
  } // namespace impl
  ...
  void localmax(I input, J& output, const SE& se, const BorderManager& bm, const ValueSet& vs)
  {
    ...
    if constexpr (SE::decomposable::value && details::is_se_compatible_with_v<SE, D>)
    {
      ...
      {
        ano::trace::warn("[Performance] Running the specialization with SE decomposition");
