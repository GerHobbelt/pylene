...
namespace ano::morpho::experimental
{
  ...
  rank_filter(InputImage&& image, const ano::StructuringElement<SE>& se, BorderManager bm);

  template <class Ratio, class InputImage, class SE, class BorderManager, class OutputImage>
  void rank_filter(InputImage&& image, const ano::StructuringElement<SE>& se, BorderManager bm, OutputImage&& out);
  ...
  template <class Ratio, class InputImage, class SE, class BorderManager, class OutputImage>
  void rank_filter(InputImage&& input, const ano::StructuringElement<SE>& se, BorderManager bm, OutputImage&& out)
  {
    using I = std::remove_reference_t<InputImage>;
    static_assert(ano::is_a<I, ano::details::Image>::value);

    using V = image_value_t<I>;
    ano_entering("ano::morpho::experimental::rank_filter");
