...
namespace ano::morpho::details
{
  ...
  {
  protected:
    virtual void partial_sum(const std::byte* in, std::byte* out, int width, int height, std::ptrdiff_t in_byte_stride,
                             std::ptrdiff_t out_byte_stride, void* sup, void* zero) = 0;
    virtual void transform(const std::byte* a, const std::byte* b, std::byte* out, int width, int height,
                           std::ptrdiff_t a_byte_stride, std::ptrdiff_t b_byte_stride, std::ptrdiff_t c_byte_stride,
                           void* sup)                                               = 0;
    void         run(int e_size, std::byte* in, int width, int height, std::ptrdiff_t byte_stride, int k, void* sup,
                     void* zero);
  };
  ...
  private:
    ...
    {
      block_partial_sum((const T*)in, (T*)out, width, height, in_byte_stride, out_byte_stride, *(BinaryFunction*)sup,
                        *(T*)zero);
    }
    ...
    void transform(const std::byte* a, const std::byte* b, std::byte* out, int width, int height,
                   ...
                   void* sup) final
    {
      block_transform((const T*)a, (const T*)b, (T*)out, width, height, a_byte_stride, b_byte_stride, out_byte_stride,
                      *(BinaryFunction*)sup);
    }
