...
#include <ano/morpho/alphatree.hpp>
#include <ano/morpho/canvas/kruskal.hpp>
#include <ano/morpho/watershed.hpp>
...
namespace ano::morpho
{
  namespace details
  {
    ...
    public:
      void        on_init(int n);
      void        on_union(int p, int rp, int q, int rq, ano::dontcare_t);
      inline void on_processing(ano::dontcare_t, ano::dontcare_t, ano::dontcare_t, ano::dontcare_t, ano::dontcare_t) {}
    ...
    public:
      void        on_init(int n);
      void        on_union(ano::dontcare_t, int rp, ano::dontcare_t, int rq, int w);
      inline void on_processing(ano::dontcare_t, ano::dontcare_t, ano::dontcare_t, ano::dontcare_t, ano::dontcare_t) {}
      inline void on_finish() {}
    ...
    std::vector<std::vector<std::pair<int, image_value_t<I>>>> watershed_rag(I input, O output, N nbh, int& nlabel)
    {
      ...
      nlabel = ano::labeling::impl::local_minima(input, nbh, output, std::less<V>()) + 1;
    ...
    std::vector<std::tuple<int, int, int>> mst_waterfall(const std::vector<std::vector<std::pair<int, V>>>& rag)
    {
      ...
      ano::morpho::canvas::kruskal(flatten_graph.data(), static_cast<int>(flatten_graph.size()),
    ...
    component_tree<int> waterfall_from_mst(std::vector<std::tuple<int, int, int>>& mst, int n_vertices, N& nm)
    {
      ...
      ano::morpho::canvas::kruskal(mst.data(), static_cast<int>(mst.size()), n_vertices, viz);
  ...
  {
    static_assert(ano::is_a_v<I, ano::details::Image>);
    static_assert(ano::is_a_v<N, ano::details::Neighborhood>);

    ano_entering("ano::morpho::waterfall");
