...
#include <ano/core/image/private/ndbuffer_image_data.hpp>
#include <ano/core/image/private/ndbuffer_image_impl.hpp>
...
namespace
{
  constexpr int ANONYMOUS_NDBUFFER_MAX_DIM = 16;


  using Impl = ano::details::ndbuffer_image_impl<void, -1>;
} // namespace
...
namespace ano
{
  ...
  {
    int a_coords[ANONYMOUS_NDBUFFER_MAX_DIM];
    int b_coords[ANONYMOUS_NDBUFFER_MAX_DIM];
  ...
  {
    ano_precondition(topleft != nullptr);
    ano_precondition(sizes != nullptr);
    ano_precondition(dim > 0);
    ano_precondition(sample_type_size > 0 || sample_type != sample_type_id::OTHER);
  ...
  void __ndbuffer_image<void, -1>::resize(sample_type_id sample_type, int dim, const int topleft[], const int sizes[],
                                          const image_build_params& params)
  {
  ...
  void __ndbuffer_image<void, -1>::resize(sample_type_id sample_type, int dim, const int sizes[],
                                          const image_build_params& params)
  {
    int topleft[ANONYMOUS_NDBUFFER_MAX_DIM] = {0};
  ...
  {
    int topleft[ANONYMOUS_NDBUFFER_MAX_DIM] = {0};
    int sizes[ANONYMOUS_NDBUFFER_MAX_DIM]   = {width, height, depth};
    for (int i = 3; i < ANONYMOUS_NDBUFFER_MAX_DIM; ++i)
      sizes[i] = 1;
  ...
  void __ndbuffer_image<void, -1>::resize(sample_type_id sample_type, ConstBoxRef domain,
                                          const image_build_params& params)
  {
    int topleft[ANONYMOUS_NDBUFFER_MAX_DIM];
    int sizes[ANONYMOUS_NDBUFFER_MAX_DIM];
    int dim = domain.dim();
    for (int i = 0; i < dim; ++i)
    {
  ...
  value_set<> __ndbuffer_image<void, -1>::get_value_set() const noexcept
  {
    return {};
  }
  ...
  {
    int dp[ANONYMOUS_NDBUFFER_MAX_DIM];
  ...
  {
    int coords[ANONYMOUS_NDBUFFER_MAX_DIM];
    for (int k = 0; k < m_pdim; ++k)
  ...
  auto __ndbuffer_image<void, -1>::clip(ConstBoxRef roi) const -> __ndbuffer_image
  {
    ...
    int begin[ANONYMOUS_NDBUFFER_MAX_DIM];
    int end[ANONYMOUS_NDBUFFER_MAX_DIM];
    for (int k = 0; k < m_pdim; ++k)
    {
      begin[k] = roi.tl(k);
      end[k]   = roi.br(k);
  ...
  auto __ndbuffer_image<void, -1>::row(int y) const -> __ndbuffer_image
  {
    ...
    int begin[ANONYMOUS_NDBUFFER_MAX_DIM] = {m_axes[0].domain_begin, y};
    int end[ANONYMOUS_NDBUFFER_MAX_DIM]   = {m_axes[0].domain_end, y + 1};
  ...
  auto __ndbuffer_image<void, -1>::slice(int z) const -> __ndbuffer_image
  {
    ...
    int begin[ANONYMOUS_NDBUFFER_MAX_DIM] = {m_axes[0].domain_begin, m_axes[1].domain_begin, z};
    int end[ANONYMOUS_NDBUFFER_MAX_DIM]   = {m_axes[0].domain_end, m_axes[1].domain_end, z + 1};
