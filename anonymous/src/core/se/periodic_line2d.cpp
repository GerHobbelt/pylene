...
namespace ano::se
{
  ...
  {
    ano_precondition((k >= 0) && "The extent must be positive");

    m_delta = (delta < ano::point2d{0, 0}) ? point2d(-delta) : delta;
    m_k     = k;
  }
  ...
  ano::box2d periodic_line2d::compute_input_region(ano::box2d roi) const noexcept
  {
  ...
  ano::box2d periodic_line2d::compute_output_region(ano::box2d roi) const noexcept
  {
    int dx = std::abs(m_delta.x()) * m_k;
    int dy = std::abs(m_delta.y()) * m_k;
    roi.tl().x() += dx;
