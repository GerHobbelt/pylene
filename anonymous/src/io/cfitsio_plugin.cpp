...
#include <ano/io/fits/imread.hpp>
#include <ano/io/private/cfitsio_plugin.hpp>
...
namespace ano::io::fits
{
  namespace internal
  {
    namespace
    {
      std::pair<ano::sample_type_id, int> get_type_info(int type)
      {
        switch (type)
        {
        case BYTE_IMG:
          return {ano::sample_type_id::UINT8, TBYTE};
        case SHORT_IMG:
          return {ano::sample_type_id::INT16, TSHORT};
        case LONG_IMG:
          return {ano::sample_type_id::INT32, TINT};
        case LONGLONG_IMG:
          return {ano::sample_type_id::INT64, TLONGLONG};
        case FLOAT_IMG:
          return {ano::sample_type_id::FLOAT, TFLOAT};
        case DOUBLE_IMG:
          return {ano::sample_type_id::DOUBLE, TDOUBLE};
          break;
        default:
      ...
      struct impl_cfitsio_t : ano::io::internal::plugin_base::impl_t
      {
    ...
    cfitsio_reader_plugin::~cfitsio_reader_plugin()
    {
      this->close();
    }
    ...
    void cfitsio_reader_plugin::open(const char* filename)
    {
      ...
      if (ndim == 0 || ndim > 4)
        throw std::runtime_error(
            fmt::format("Unhandled image number of dimension (Got {}, expected in [1 - 4])", ndim));
  ...
  ano::ndbuffer_image imread(const std::string& filename, int ind)
  {
  ...
  void imread(const std::string& filename, ano::ndbuffer_image& out, int ind)
  {
