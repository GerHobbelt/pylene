    ...
    struct impl_bit_t : impl_base_t
    {
      ...
      void write_next_line(const std::byte* __restrict buffer_) final
      {
        ...
        if (remainder > 0)
        {
          ...
          {
          case 7:
            lineptr[t] |= buffer[x + 6] << 1;
            [[fallthrough]];
          case 6:
            lineptr[t] |= buffer[x + 5] << 2;
            [[fallthrough]];
          case 5:
            lineptr[t] |= buffer[x + 4] << 3;
            [[fallthrough]];
          case 4:
            lineptr[t] |= buffer[x + 3] << 4;
            [[fallthrough]];
          case 3:
            lineptr[t] |= buffer[x + 2] << 5;
            [[fallthrough]];
          case 2:
            lineptr[t] |= buffer[x + 1] << 6;
            [[fallthrough]];
          case 1:
            lineptr[t] |= buffer[x + 0] << 7;
          }
        }
        m_y--;
      }
