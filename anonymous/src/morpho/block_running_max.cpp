...
namespace ano::morpho::details
{
  ...
  void block_running_max_algo_base::run(int e_size, std::byte* in, int width, int height, std::ptrdiff_t byte_stride,
                                        int k, void* sup, void* zero)
  {
    ...
    //        = Max( h[x-k], g[x+k] )
    this->transform(g + k * kBlockLineByteSize, h - k * kBlockLineByteSize, in, width, height, kBlockLineByteSize,
                    kBlockLineByteSize, byte_stride, sup);
    std::free(g0);
    std::free(h0);
  }
