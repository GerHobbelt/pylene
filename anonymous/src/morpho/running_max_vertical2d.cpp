...
namespace ano::morpho::details
{
  ...
  void vertical_running_max_algo_base_t::running_max_block2d(std::byte* __restrict f, std::byte* __restrict g,
                                                             ...
                                                             ano::box2d roi, int k, bool use_extension)
  {
    int x0     = roi.x();
    int y0     = roi.y();
    int width  = roi.width();
...

    {
      ...
      for (; rem > 0; chunk_start += alpha, rem -= alpha)
      {
        ...
        if (m_tile_loader)
        {
          ano::box2d region(x0, y0 + chunk_start, width, chunk_size);
    ...
    //        = Max( h[x-k], g[x+k] )
    {
      ...
      for (int y = 0; y < height; y += kBlockHeight)
      {
        ...
        if (m_tile_writer)
        {
          ano::box2d region(x0, y0 + y, width, hroi);
  ...
  void vertical_running_max_algo_base_t::execute(ano::box2d roi, int k, bool use_extension, bool vertical)
  {
    ...
    for (int x = 0; x < width; x += kBlockWidth)
    {
      ...
      ano::box2d region(x0 + x, y0, w, height);
