...
int main(int argc, char** argv)
{
  ...
  const char* usage = "Use the energy E(Γ) = α V₁(Γ) + βV₂(Γ) + V₃(Γ)\n"
                      "\twith\n"
  ...
  desc.add_options()("params,p", po::value<std::vector<float>>()->multitoken()->required(), "ε α β γ");
  ...
  // V2
  auto amap2 =
      compute_attribute_on_contour(tree, C, accu::features::count<unsigned>() & accu::features::mean<double>());
  ...
  const char* names[] = {"VIN", "VOUT", "VTOTAL", "V1", "Cont. Length", "Curv. Avg", "Energy", "Extinction"};
  ...
  auto v1     = make_functional_property_map<tree_t::node_type>(
      [&amap](tree_t::node_type x) { return amap[x][2] == 0 ? 0 : (amap[x][0] + amap[x][1]) / amap[x][2]; });

  auto cont_length = make_functional_property_map<tree_t::node_type>(
      [&amap2](tree_t::node_type x) { return accu::extractor::count(amap2[x]); });
  auto cont_curv = make_functional_property_map<tree_t::node_type>(
      [&amap2](tree_t::node_type x) { return accu::extractor::mean(amap2[x]); });

  std::function<float(tree_t::node_type)> attrs[] = {_as_fun(vin),    _as_fun(vout),        _as_fun(vtotal),
                                                     _as_fun(v1),     _as_fun(cont_length), _as_fun(cont_curv),
                                                     _as_fun(energy), _as_fun(smap)};
