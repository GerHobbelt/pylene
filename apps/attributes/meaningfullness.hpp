...
namespace mln
{
  ...
  image2d<float> meaningfullness(const image2d<V>& ima, const image2d<T>& K, const image2d<unsigned>& parent,
  ...
  template <typename V>
  image2d<float> close(const image2d<float>& energy, const image2d<V>& K, const image2d<unsigned>& parent,
  ...
  namespace internal
  {
    ...
    struct energy_t
    {
      ...
      static std::pair<float, float> inter(float m1, float var1, float m2, float var2, float& delta)
      {
        float a = var2 - var1;
      ...
      friend std::ostream& operator<<(std::ostream& os, const energy_t& acc)
      {
    ...
    template <typename V>
    unsigned common_ancestor(unsigned x, unsigned y, const image2d<energy_t<V>>& e, const image2d<unsigned>& parent)
    {
      while (x != y)
      {
    ...
    unsigned common_ancestor(Iterator x, const image2d<energy_t<V>>& e, const image2d<unsigned>& parent,
                             const image2d<T>& K)
    {
  ...
  template <typename V>
  auto norm(const V& v) ->
      typename std::enable_if<!std::is_scalar<V>::value, decltype(std::abs(v[0]) + std::abs(v[0]))>::type
  {
    auto s = std::abs(v[0]);
  ...
  template <typename V>
  image2d<float> curvature_on_edge(const image2d<V>& ima_)
  {
    static_assert(std::is_same<V, uint8>::value or std::is_same<V, rgb8>::value, "V must either uint8 or rgb8.");

    struct to_uint8
    {
      static image2d<uint8> foo(const image2d<uint8>& x) { return x; }

      static image2d<uint8> foo(const image2d<rgb8>& ima_)
      {
    ...
    mln_foreach (const point2d& p, ima.domain())
    {
      // Right edge
      {
        ...
        float uy = (ima.at(p + point2d{1, 0}) - ima.at(p + point2d{-1, 0}) + ima.at(p + point2d{1, 1}) -
                    ima.at(p + point2d{-1, 1})) /
                   4.0;

        float uxx =
            (ima.at(p + point2d{0, -1}) - ima.at(p) - ima.at(p + point2d{0, 1}) + ima.at(p + point2d{0, 2})) / 2.0;

        float uxy = (-ima.at(p + point2d{1, 0}) + ima.at(p + point2d{-1, 0}) + ima.at(p + point2d{1, 1}) -
                     ima.at(p + point2d{-1, 1})) /
                    2.0;

        float uyy = (ima.at(p + point2d{-1, 0}) + ima.at(p + point2d{-1, 1}) + ima.at(p + point2d{1, 0}) +
      ...
      // Bottom edge
      {
        ...
        float ux = (ima.at(p + point2d{0, 1}) - ima.at(p + point2d{0, -1}) + ima.at(p + point2d{1, 1}) -
                    ima.at(p + point2d{1, -1})) /
                   4.0;

        float uyy =
            (ima.at(p + point2d{-1, 0}) - ima.at(p) - ima.at(p + point2d{1, 0}) + ima.at(p + point2d{2, 0})) / 2.0;

        float uxx = (ima.at(p + point2d{0, -1}) + ima.at(p + point2d{1, -1}) - 2 * ima.at(p) -
                     2 * ima.at(p + point2d{1, 0}) + ima.at(p + point2d{0, 1}) + ima.at(p + point2d{1, 1})) /
                    2.0;

        float uxy = (ima.at(p + point2d{0, -1}) - ima.at(p + point2d{0, 1}) - ima.at(p + point2d{1, -1}) +
                     ima.at(p + point2d{1, 1})) /
                    2.0;
  ...
  template <typename V, typename T>
  image2d<float> compute_energy_(const image2d<V>& ima, const image2d<T>& K, const image2d<unsigned>& parent_,
                                 const std::vector<unsigned>& S, float alpha, float beta, float gamma, int eps = 5,
  ...
  template <typename V, typename T>
  image2d<float> meaningfullness(const image2d<V>& ima, const image2d<T>& K, const image2d<unsigned>& parent_,
                                 const std::vector<unsigned>& S, float alpha, float beta, float gamma, int eps)
  {
  ...
  template <typename V, typename T>
  image2d<float> meaningfullness(const image2d<V>& ima, const image2d<T>& K, const image2d<unsigned>& parent_,
                                 const std::vector<unsigned>& S, image2d<internal::energy_t<V>>& acc, float alpha,
                                 float beta, float gamma, int eps = 5)
  {
