...
int main(int argc, char** argv)
{
...

  {
    ...
    mln_foreach (auto x, tree.nodes())
      f << accu::extractor::count(res[x]) << "," << accu::extractor::mean(res[x]) << ","
        << accu::extractor::variance(res[x]) << "," << accu::extractor::count(res2[x]) << ","
        << accu::extractor::mean(res2[x]) << "," << accu::extractor::variance(res2[x]) << "\n";
  }
}
