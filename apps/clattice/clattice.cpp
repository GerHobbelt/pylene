...
void showstat(const ublas::triangular_matrix<bool, ublas::upper>& mat)
{
...

  {
    ...
    std::cout << "Profondeur min moyenne: " << (s / n_) << " - std: " << std::sqrt(s_2 / n_ - sqr(s / n_)) << std::endl;
  }
  {
    unsigned s = 0, s_2 = 0;
    for (int v : profmax)
    ...
    std::cout << "Profondeur max moyenne: " << (s / n_) << " - std: " << std::sqrt(s_2 / n_ - sqr(s / n_)) << std::endl;
  }
}
             ...
             LowerCompare cmp_1 = LowerCompare(), UpperCompare cmp_2 = UpperCompare())
{
  ...
  auto idx2rgb = [](unsigned x) {
    return rgb8{(uint8)((x & 0xFF0000) >> 16), (uint8)((x & 0x00FF00) >> 8), (uint8)(x & 0x0000FF)};
  };
  {
  ...
  if (params.display)
  {
    std::sort(vshapes.begin(), vshapes.end(), [](const shape_t& x, const shape_t& y) { return x.size() < y.size(); });
...
int main(int argc, char** argv)
{
  ...
  desc.add_options()("help", "produce help message")("noborder", po::bool_switch(), "add a border to the image")(
      "immerse", po::bool_switch(&params.immerse), "immerse in range value")(
      "subquant", po::value<int>(&params.subquant)->default_value(0),
      "subquantization factor")("display", po::bool_switch(&params.display), "Display shape at the end")(
      "stat", po::bool_switch(&params.stat), "Display stat at the end")("filter", po::bool_switch(&params.filter),
                                                                        "Display stat at the end")(
      "grain", po::value<std::vector<int>>(&params.filtersizes)->multitoken(),
      "Grain sizes")("export", po::bool_switch(), "Export the lattice as a dot graph.")(
      "export-filename", po::value<std::string>(&params.export_filename)->default_value("output.dot"),
      "Dot filename (if it exists, override)")("export-dir", po::value<std::string>(&params.export_dir),
                                               "Directory to store shape images (if it exists, override).")(
