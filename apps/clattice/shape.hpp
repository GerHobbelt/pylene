...
public:
  ...
  shape(V level, UpperCompare cmp, int sz, int ncols, typename std::enable_if<hasUL, dummy>::type* = NULL);
  ...
  void set_level(V level, UpperCompare cmp, typename std::enable_if<hasUL, dummy>::type* = NULL);
...
struct shape<V, LowerCompare, UpperCompare>::set_iterator
    : boost::iterator_facade<shape::set_iterator, mln::point2d, boost::forward_traversal_tag, mln::point2d>
{
  set_iterator(const boost::dynamic_bitset<>* bset, int ncols, boost::dynamic_bitset<>::size_type first)
      : m_bset(bset), m_nc(ncols), m_cur(first)
  {
  }
...
private:
  ...
  void increment() { m_cur = m_bset->find_next(m_cur); }

  bool equal(const set_iterator& other) const { return this->m_cur == other.m_cur; }

  mln::point2d dereference() const { return {(short)(m_cur / m_nc), (short)(m_cur % m_nc)}; }
...
namespace impl
{
  inline bool is_box_inc(const mln::box2d& a, const mln::box2d& b)
  {
...
is_shape_included(const shape<V, LowerCompare, UpperCompare>& a, const shape<V, LowerCompare, UpperCompare>& b)
{
  ...
  {
    LowerCompare cmp;
    if (a.lower_level() == b.lower_level() or cmp(a.lower_level(), b.lower_level()))
      return b.m_set.test(a.m_first);
  }
  if (a.isupper() and b.isupper() and !std::is_same<LowerCompare, UpperCompare>::value)
  {
    UpperCompare cmp;
    if (a.upper_level() == b.upper_level() or cmp(a.upper_level(), b.upper_level()))
      return b.m_set.test(a.m_first);
  }
...
template <typename V, class LowerCompare, class UpperCompare>
shape<V, LowerCompare, UpperCompare>::shape(V x, LowerCompare, int sz, int ncols)
    : m_type(LOWER), m_level_1(x), m_size(sz), m_nc(ncols)
{
  init();
}
...
shape<V, LowerCompare, UpperCompare>::shape(V x, UpperCompare, int sz, int ncols,
                                            typename std::enable_if<hasUL, dummy>::type*)
    : m_type(UPPER), m_level_2(x), m_size(sz), m_nc(ncols)
{
  init();
}
...
shape<V, LowerCompare, UpperCompare>::shape(shape&& other)
    : m_type(other.m_type), m_level_1(other.m_level_1), m_level_2(other.m_level_2), m_cmp_1(other.m_cmp_1),
      m_cmp_2(other.m_cmp_2), m_size(other.m_size), m_nc(other.m_nc), m_bbox(other.m_bbox), m_first(other.m_first),
...
operator<<(std::ostream& os, const shape<V, LowerCompare, UpperCompare>& shp)
{
  ...
  if (shp.islower())
  {
    os << "\tfrom: [u" << LowerCompare::str << (shp.isleq() ? '=' : ' ');
    format(os, shp.lower_level()) << "]";
  }
  if (shp.isupper())
  {
    os << "\tfrom: [u" << UpperCompare::str << (shp.isgeq() ? '=' : ' ');
...
prettyprint_shape(const shape<V, LowerCompare, UpperCompare>& shp, const mln::box2d& domain)
{
  ...
  std::cout << "==Info== Size:" << shp.size() << "(0x" << std::hex << shp.size() << ")" << std::dec << std::endl;

  if (shp.islower())
  {
    std::cout << "\tfrom: [u" << LowerCompare::str << (shp.isleq() ? '=' : ' ');
    format(std::cout, shp.lower_level()) << "]" << std::endl;
  }

  if (shp.isupper())
  {
    std::cout << "\tfrom: [u" << UpperCompare::str << (shp.isgeq() ? '=' : ' ');
    format(std::cout, shp.upper_level()) << "]" << std::endl;
  }

  std::cout << "hash: " << std::hex << "0x" << std::hash<shape<V, LowerCompare, UpperCompare>>()(shp) << std::dec
            << std::endl;
...
namespace std
{
  ...
  struct hash<shape<V, LowerCompare, UpperCompare>>
  {
    ...
    size_t operator()(const shape_t& x) const
    {
      size_t h = ((x.size() & 0x00FF) << 24) | ((x.bbox().pmin[0] & 0x00FF) << 16) |
                 ((x.bbox().pmin[1] & 0x00FF) << 8) | ((x.bbox().pmax[0] + x.bbox().pmax[1]) & 0x00FF);
      return h;
    }
  };
}
