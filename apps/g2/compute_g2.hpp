...
namespace mln
{
  ...
  std::tuple<Graph<NTREE>, std::array<tlink_t, NTREE>> compute_g2(const tree_t* trees);

  // Explicit Declaration
  extern template std::tuple<Graph<2>, std::array<tlink_t, 2>> compute_g2<2>(const tree_t* trees);

  extern template std::tuple<Graph<3>, std::array<tlink_t, 3>> compute_g2<3>(const tree_t* trees);

  extern template std::tuple<Graph<4>, std::array<tlink_t, 4>> compute_g2<4>(const tree_t* trees);
