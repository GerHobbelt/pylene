...
namespace mln
{
  ...
  boost::vector_property_map<unsigned> compute_graph_count(const MyGraph& g)
  {

    struct viz_t : public boost::default_dfs_visitor
    {
      viz_t(boost::vector_property_map<unsigned>& common) : m_common(common) {}
  ...
  /// \[ ω(x) = { X ∈ S, x ∈ X } \]
  image2d<unsigned> compute_map_count(const MyGraph& g, const tree_t* t, const tlink_t* tlink)
  {
  ...
  template <typename V>
  boost::vector_property_map<unsigned> compute_graph_variation(const MyGraph& g,
  ...
  template <typename V>
  boost::vector_property_map<float> compute_graph_area(const MyGraph& g, const image2d<V>& ima, const tree_t* trees,
  ...
  template <class ValueMap, class ColorMap, class Compare, class ValueType, class ColorType>
  void write_vmap_to_image_and_rec(const MyGraph& g, const tree_t* t, const tlink_t* tlink, const ValueMap& vmap,
                                   const ColorMap& cmap, Compare cmp, image2d<ValueType>& vout,
...
main(int argc, char** argv)
{
  ...
  std::map<std::string, e_opt> str2opt = {{"depth", OPT_DEPTH},
                                          {"count", OPT_COUNT},
                                          {"count2", OPT_COUNT2},
  ...
  tbb::parallel_for(0, (int)NTREE, [&t, &f](int i) { t[i] = morpho::maxtree_indexes(eval(channel(f, i)), c4); });
  ...
  if (attribute == OPT_DEPTH or attribute == OPT_COUNT2)
  {
    ...
    if (argc > 0)
    {
      ...
      {
        auto d = make_functional_property_map<tree_t::node_type>(
            [&tlink, &gdepth, i](tree_t::node_type x) { return gdepth[tlink[i][x]]; });
    ...
    for (int i = 0; i < NTREE; ++i)
      vmap[i] = morpho::make_attribute_map_from_image(t[i], imtransform(F, [i](value_t x) { return x[i]; }));
  ...
  else if (attribute == OPT_VARIATION)
  {
    ...
    if (argc > 0)
    {
      ...
      for (int i = 0; i < std::min<int>(argc, NTREE); ++i)
      {
        auto vmap = morpho::make_attribute_map_from_image(t[i], imtransform(F, [i](value_t x) { return x[i]; }));
