...
namespace mln
{
  void mystretch(const image2d<vec<unsigned, NTREE>>& f, const char* name)
  {
    image2d<value_t> out;
    resize(out, f);
...
int main(int argc, char** argv)
{
  ...
  auto cvrter = [cvrter_fun](const image2d<depth_vec_t>& f) { return imtransform(f, cvrter_fun); };
  ...
  // Debug
  {
    for (int k = 0; k < NTREE; ++k)
    {
      auto dmap = make_functional_property_map<tree_t::node_type>(
          [&d, &tlink, k](tree_t::node_type x) { return d[tlink[k][x]][k]; });
