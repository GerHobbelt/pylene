...
namespace mln
{
  ...
  void write_graphviz(const std::string& filename, const tree_t& t1, const tree_t& t2, const tree_t& t3,
                      ...
                      const property_map<tree_t, V>& val1, const property_map<tree_t, V>& val2,
                      const property_map<tree_t, V>& val3, const Graph& graph, const GraphValueMap& vmap)
  {
    using mln::io::format;
...
void make_minimum_spanning_tree(Graph& g, const WeightPropertyMap& weights)
{
  ...
  boost::remove_edge_if(
      [&tg, &parent](const Graph::edge_descriptor& e) { return parent[boost::source(e, tg)] != boost::target(e, tg); },
      g);
...
boost::vector_property_map<unsigned> compute_graph_depth(const Graph& g)
{
  ...
  boost::dag_shortest_paths(
      gT, root,
      boost::weight_map(w).distance_map(depth).distance_compare(std::greater<int>()).distance_inf(-1).distance_zero(0));
...
int main(int argc, char** argv)
{
  if (argc < 3)
  {
    std::cerr << "Usage: " << argv[0] << " input.ppm grain" << std::endl;
    std::exit(1);
  }
  ...
  // Save depth images
  {
    ...
    image2d<unsigned> maxdepth = write_vmap_to_image(
        g2, gmap, gdepth, (const unsigned& (*)(const unsigned&, const unsigned&))std::max<unsigned>, 0u);
  ...
  for (int i = 2; i < argc; ++i)
  {
    ...
    boost::filtered_graph<Graph, boost::keep_all, std::function<bool(Graph::vertex_descriptor)>> fg(
        g2, boost::keep_all(), gpred);
