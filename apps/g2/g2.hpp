...
namespace mln
{
	     ...
	     const tree_t& t3)
  {
	...
	ulink[v] = node;
	gdepth[v][0] = d1[node];
	gdepth[v][1] = d2[SES_12[node]];
	gdepth[v][2] = d3[SES_13[node]];
	...
	if (SES_12[SES_21[node]] == node) // Already inserted
	  ...
	  glink[v][0] = t1.nend();
	  glink[v][2] = t3.nend();
	  gdepth[v][0] = d1[SES_21[node]];
	  gdepth[v][1] = d2[node];
	  gdepth[v][2] = d3[SES_23[node]];
	  ulink[v] = node;
	}
	...
	if (SES_13[SES_31[node]] == node) // Already inserted
	  ...
	  glink[v][0] = t1.nend();
	  glink[v][1] = t2.nend();
	  gdepth[v][0] = d1[SES_31[node]];
	  gdepth[v][1] = d2[SES_32[node]];
	  gdepth[v][2] = d3[node];
	  ulink[v] = node;
	}
    ...
    // {
    //   std::ofstream outs("out-closure.dot");
    //   auto gpw = [&](std::ostream& os, const Graph::vertex_descriptor &v) {
    // 	os << (boost::format("[label=\"%s\\n%s\"]") % v % gdepth[v]);
    //   };

    //   boost::write_graphviz(outs, graph, gpw);
    //   outs.close();
    // }
