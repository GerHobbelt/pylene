...
namespace mln
{
  ...
  /// \pre \p g must be a tree
  template <class Graph>
  void
  update_parent_relation(const Graph& g, tree_t& t1, tree_t& t2, tree_t& t3, tlink_t& t1link);



  template <class Graph>
  void
  update_parent_relation(const Graph& g, tree_t& t1, tree_t& t2, tree_t& t3,
			 tlink_t& t1link, tlink_t& t2link, tlink_t& t3link)
  {
	...
	vtype s = boost::source(e, g);
	vtype t = boost::target(e, g);

	for (int k = 0; k < 3; ++k)
	  {
	    if (glink[t][k].id() != tree_t::npos())
	      parent[s][k] = t;
      ...
      while (!cur.finished())
	{
	  ...
	  // std::cout << tlink[x.id()] << " par: " << tlink[x.parent().id()]
	  // 	      << " newpar: " << par << std::endl;
	  if (newpar != x.parent())
	    {
