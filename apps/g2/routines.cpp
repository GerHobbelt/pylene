...
namespace mln
{
  ...
  void compute_graph_map(const MyGraph& g2, const tree_t& t1, const tree_t& t2, const tree_t& t3, const tlink_t& t1link,
                         const tlink_t& t2link, const tlink_t& t3link, image2d<vec3u>& out)
  {
    ...
    mln_forall (px)
    {
      ...
      auto inc = [n, &gnode, &SES](int a, int b) { return SES[gnode[a]][b] == n[b]; };
  ...
  void compute_graph_map(const MyGraph& g2, const tree_t* t, const tlink_t* tlink, image2d<vec<unsigned, NTREE>>& out)
  {
    mln_entering("mln::compute_graph_map");
