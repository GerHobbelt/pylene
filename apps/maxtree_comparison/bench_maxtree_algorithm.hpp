...
#define MAXTREE_ALGO(Name, Fun)                                                                                        \
  ...
  {                                                                                                                    \
    ...
    auto operator()(const I& x, const N& y, const S& z) -> decltype(Fun(x, y, z))                                      \
    {                                                                                                                  \
      return Fun(x, y, z);                                                                                             \
    }                                                                                                                  \
  };
...
void run_test(int argc, char** argv, Algorithm algo)
{
  ...
  desc.add_options()("help", "produce help message")(
      "nthread", po::value<int>()->default_value((int)tbb::task_scheduler_init::automatic),
      "set number of thread (default: auto)")("nbits", po::value<int>()->default_value(8),
      ...
      "ntest", po::value<int>()->default_value(3),
      "set number of runs (default: 3)")("sz", po::value<int>()->default_value(0),
  ...
  hidden.add_options()("input-file", po::value<std::string>()->required(), "input");
