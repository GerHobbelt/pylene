    ...
    namespace impl
    {
      ...
      struct MaxTreeAlgorithmHQ
      {
        ...
        void operator()(const box2d& domain)
        {
          ...
          if (parallel)
            internal::maxtree_flood_algorithm<image2d<V>, Neighborhood, StrictWeakOrdering, parallel>::run(
                ima, parent, m_nbh, m_cmp, NULL);
          else
            internal::maxtree_flood_algorithm<image2d<V>, Neighborhood, StrictWeakOrdering, parallel>::run(
                ima, parent, m_nbh, m_cmp, &m_S[0] + domain.size());
