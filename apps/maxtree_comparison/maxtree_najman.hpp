...
namespace mln
{
  ...
  namespace morpho
  {
    ...
    namespace internal
    {
      ...
      static inline size_type zfindroot(image2d<size_type>& parent, size_type p)
      {
        if (parent[p] == p)
          return p;
        else
      ...
      template <typename size_type>
      static inline size_type zfindroot(image2d<aux_data<size_type>>& aux, size_type p)
      {
      ...
      template <typename size_type>
      static inline size_type mergeset(image2d<aux_data<size_type>>& aux, size_type p, size_type q)
      {
        if (aux[p].rank > aux[q].rank)
          std::swap(p, q);
        if (aux[p].rank == aux[q].rank)
          aux[q].rank += 1;
        aux[p].zpar = q;
