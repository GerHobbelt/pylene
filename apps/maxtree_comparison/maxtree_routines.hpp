...
namespace mln
{
  ...
  namespace morpho
  {
    ...
    namespace internal
    {
      ...
      template <typename V>
      point2d zfind_repr(const image2d<V>& ima, image2d<point2d>& parent, const point2d& p)
      {
        point2d q = parent(p);
      ...
      template <typename V>
      unsigned zfind_repr(const image2d<V>& ima, image2d<typename image2d<V>::size_type>& parent,
      ...
      template <typename V>
      std::size_t zfind_parent(const image2d<V>& ima, image2d<typename image2d<V>::size_type>& parent,
                               typename image2d<V>::size_type p)
      {
      ...
      template <typename V>
      std::size_t zfind_root(const image2d<V>& ima, image2d<typename image2d<V>::size_type>& parent,
                             typename image2d<V>::size_type p)
      {
        typename image2d<V>::size_type q = parent[p];
        if (q != p)
    ...
    struct MaxtreeCanonizationAlgorithm
    {
      ...
      void operator()(const box2d& domain) const
      {
