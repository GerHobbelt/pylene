  ...
  namespace morpho
  {
    ...
    namespace impl
    {
      ...
      struct MaxTreeAlgorithmUF
      {
        ...
        static inline size_type zfindroot(image2d<size_type>& parent, const size_type& p)
        {
          if (parent[p] == p)
            return p;
          else
        ...
        void unionfind(const box2d& domain)
        {
      ...
      namespace parallel
      {
        ...
        template <typename V, typename Neighborhood, typename StrictWeakOrdering = std::less<V>>
        std::pair<image2d<typename image2d<V>::size_type>, std::vector<typename image2d<V>::size_type>>
            maxtree_ufind_line(const image2d<V>& ima, const Neighborhood& nbh,
