...
namespace mln
{
  ...
  namespace morpho
  {
    ...
    namespace impl
    {
      ...
      struct MaxTreeAlgorithmUFRank
      {
        ...
        MaxTreeAlgorithmUFRank(MaxTreeAlgorithmUFRank& other, tbb::split)
            : m_ima(other.m_ima), m_nbh(other.m_nbh), m_cmp(other.m_cmp), m_parent(other.m_parent), m_aux(other.m_aux),
              m_has_previous(false), m_S(other.m_S)
        {
          m_nsplit = 0;
        }

        size_type zfindroot(image2d<aux_data>& aux, size_type p)
        {
        ...
        void unionfind(const grain_box2d& domain)
        {
          ...
          for (int i = ima.domain().size() - 1; i >= 0; --i)
          {
            ...
            for (unsigned j = 0; j < dindexes.size(); ++j)
            {
              ...
              if (processed)
              {
                ...
                if (r != z) // make union
                {
                  if (level_compression and
                      ima[aux[r].repr] == ima[x]) // FIXME: use !(b>a) for a==b because ima[x]<=ima[repr[r]]
                  ...
                  if (aux[z].rank < aux[r].rank)
                  { // we merge z to r
                    aux[z].zpar = r;
                    aux[r].repr = x;
                    z = r;
                  }
                  else if (aux[z].rank > aux[r].rank)
                  { // merge r to z
                    aux[r].zpar = z;
                    aux[z].repr = x;
                  }
    ...
    // template <typename V, typename Neighborhood, typename StrictWeakOrdering = std::less<V> >
    // image2d<point2d>
    // maxtree_ufind_parallel(const image2d<V>& ima, const Neighborhood& nbh, StrictWeakOrdering cmp =
    // StrictWeakOrdering())
    // {
