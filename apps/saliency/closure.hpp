...
namespace internal
{
  ...
  template <typename V>
  mln::image2d<child_t> getchilds(const mln::image2d<V>& K, const mln::image2d<unsigned>& parent,
  ...
  template <typename V>
  std::vector<unsigned> getnodes(const mln::image2d<V>& K, const mln::image2d<unsigned>& parent,
                                 const std::vector<unsigned>& S)
  {
...
#define INTERNAL_ZFINDROOT
  ...
  unsigned zfindroot_(mln::image2d<unsigned>& par, unsigned x)
  {
    if (par[x] != x)
