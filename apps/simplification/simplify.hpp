...
namespace mln
{
  ...
  image2d<V> simplify_bottom_up(const image2d<V>& f, const image2d<T>& K, const image2d<unsigned>& parent,
                                const std::vector<unsigned>& S, int lambda, int area = 0, float areafactor = 0);

  template <typename V, typename T>
  image2d<V> simplify_top_down(const image2d<V>& f, const image2d<T>& K, const image2d<unsigned>& parent,
  ...
  template <typename T>
  image2d<unsigned> smallest_enclosing_shape(const image2d<T>& K, const image2d<unsigned>& parent,
                                             const std::vector<unsigned>& S, int eps,
  ...
  namespace internal
  {
    ...
    unsigned common_ancestor(unsigned x, const Neighborhood<N>& nbh_, const image2d<T>& K,
    ...
    unsigned common_ancestor(unsigned x, unsigned y, const image2d<unsigned>& parent, const image2d<unsigned>& depth)
    {
      while (x != y)
      {
        if (depth[x] > depth[y])
  ...
  template <typename T>
  image2d<unsigned> smallest_enclosing_shape(const image2d<T>& K, const image2d<unsigned>& parent,
                                             const std::vector<unsigned>& S, int eps, const image2d<unsigned>* depth_)
  {
  ...
  template <typename V, typename T>
  image2d<V> simplify_bottom_up(const image2d<V>& f, const image2d<T>& K, const image2d<unsigned>& parent,
                                const std::vector<unsigned>& S, int eps, int area, float areafactor)
  {
    ...
    // io::imsave(transform(enc, [&areas, &S](unsigned x) -> float { return (float) areas[x] / areas[S[0]]; }),
    // "tmp.tiff");
  ...
  image2d<V> simplify_top_down_pred(const image2d<V>& f, const image2d<T>& K, const image2d<unsigned>& parent,
  ...
  template <typename V, typename T>
  image2d<V> simplify_top_down_tolerance(const image2d<V>& f, const image2d<T>& K, image2d<unsigned>& parent,
