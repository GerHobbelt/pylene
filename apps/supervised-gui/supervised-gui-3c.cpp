                                          ...
                                          unsigned labelization_policy)
{
  ...
  if ((labelization_policy & AMBIGUITY_MASK) != AMBIGUITY_AS_MAJORITY)
  {
    ...
    {
    case AMBIGUITY_AS_FOREGROUND:
      dummy_label = FOREGROUND;
      break;
    case AMBIGUITY_AS_BACKGROUND:
                           ...
                           const image2d<rgb8>& markers__, float reject = 0.5, unsigned policy = 0)
{
  ...
  auto cmap = make_functional_property_map<tree_t::node_type>([&dmap, reject](const tree_t::node_type& x) -> uint8 {
...
int main(int argc, char** argv)
{
  ...
  QObject::connect(threshold_widget, SIGNAL(textEdited(const QString&)), &brush,
                   SLOT(set_reject_value(const QString&)));
  QObject::connect(ambiguity_policy_selector, SIGNAL(currentIndexChanged(int)), &brush,
                   SLOT(set_ambiguity_policy(int)));
