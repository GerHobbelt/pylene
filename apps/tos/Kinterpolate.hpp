...
namespace mln
{
  ...
  template <typename U, typename V = decltype(std::declval<U>() * 2), class Compare = std::less<U>>
  image2d<V> interpolate_median(const image2d<U>& ima, const V& = V(), const Compare& = Compare());
  ...
  template <typename I>
  image2d<mln_value(I)> immerse_k1(const Image<I>& ima, mln_value(I) v = mln_value(I)());
  ...
  image2d<V> interpolate_median(const image2d<U>& ima, const V&, const Compare& cmp)
  {
  ...
  template <typename I>
  image2d<mln_value(I)> immerse_k1(const Image<I>& ima_, mln_value(I) v)
  {
  ...
  image2d<T> Kadjust_to(const image2d<T>& ima, box2d domain, const std::string& method)
  {
    ...
    if (shp + 2 == shp0)
    { // remove border
      subdomain = sbox2d{ima.domain().pmin + point2d{1, 1}, ima.domain().pmax - point2d{1, 1}, {1, 1}};
    }
    else if (shp * 2 - 1 == shp0)
    { // unimmerse k1
      subdomain = sbox2d{ima.domain().pmin, ima.domain().pmax, {2, 2}};
    }
    else if (shp * 2 + 3 == shp0)
    { // unimmerse k1 - border
      subdomain = sbox2d{ima.domain().pmin + point2d{2, 2}, ima.domain().pmax - point2d{2, 2}, {2, 2}};
    }
    else if (shp * 4 - 3 == shp0)
    { // unimmerse k2
      subdomain = sbox2d{ima.domain().pmin, ima.domain().pmax, {4, 4}};
    }
    else if (shp * 4 + 5 == shp0)
    { // unimmerse k2 - border
      subdomain = sbox2d{ima.domain().pmin + point2d{4, 4}, ima.domain().pmax - point2d{4, 4}, {4, 4}};
    }
    else if (shp == shp0)
    {
      return ima;
