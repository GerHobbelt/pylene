...
namespace mln
{
  ...
  template <typename T>
  void displayTree(const image2d<T>& ima, const image2d<typename image2d<T>::size_type>& parent,
                   const std::vector<typename image2d<T>::size_type>& S, const point2d strides = point2d{1, 1})
  {
...
void usage(int argc, char** argv)
{
      ...
      (argv[2] != std::string("min") && argv[2] != std::string("max")))
  {
    std::cerr << "Usage: " << argv[0]
              << "(mintree|tos) (min|max) ima.(ppm|png|tiff...) lambda1 [lambda2  [lambda3 [...]]]" << std::endl
