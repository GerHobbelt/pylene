...
usage(int argc, char** argv)
{
  if (argc < 4)
  {
    std::cout << "Usage: " << argv[0] << "[--lsh] input.(ppm|png...) out_without_extension grain1 [grain2 [grain3]]"
              << std::endl
...
namespace mln
{
   ...
   * \pre the ToS must have been computed with a total Order (not a pre-order), if not the value of a node of
   * equivalents pixels is undefined \return The filtered image.
   */
  template <typename V, typename T>
  image2d<V> grainfilter(const image2d<V>& ima, const image2d<T>& K, const image2d<unsigned>& parent,
                         const std::vector<unsigned>& S, unsigned grain)
  {
  ...
  bool lex_compare(const Vec& x, const Vec& y)
  {
