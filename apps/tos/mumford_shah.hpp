...
namespace mln
{
  ...
  image2d<float> compute_energy(const image2d<V>& ima, const image2d<T>& K, const image2d<unsigned>& parent,
  ...
  {
    template <typename V>
    decltype(std::declval<V>() * 2) sqr(const V& x)
    {
      return x * x;
    }

    template <typename V>
    typename std::enable_if<std::is_scalar<V>::value, V>::type sum(const V& x)
    {
      return x;
    }
    ...
    struct energy_t
    {
      ...
      friend std::ostream& operator<<(std::ostream& os, const energy_t& acc)
      {
        return os << "Contour length: " << acc.m_e_length << std::endl
                  ...
                  << "Internal Variance: " << (sum(acc.m_v_sum_int_sqr) - sum(sqr(acc.m_v_sum_int)) / acc.m_v_n_int)
                  << std::endl
                  << "External Variance: "
                  << (acc.m_v_n_ext == 0 ? 0 : sum(acc.m_v_sum_ext_sqr) - sum(sqr(acc.m_v_sum_ext)) / acc.m_v_n_ext)
                  << std::endl
  ...
  image2d<float> curvature_on_edge(const image2d<V>& ima)
  {
    ...
    mln_foreach (const point2d& p, ima.domain())
    {
      // Right edge
      {
        ...
        float uxx = norm((vec_t)ima.at(p + point2d{0, -1}) - (vec_t)ima.at(p) - (vec_t)ima.at(p + point2d{0, 1}) +
                         (vec_t)ima.at(p + point2d{0, 2})) /
                    2.0;

        float uyy = norm((vec_t)ima.at(p + point2d{-1, 0}) + (vec_t)ima.at(p + point2d{-1, 1}) -
                         2 * ((vec_t)ima.at(p) + (vec_t)ima.at(p + point2d{0, 1})) + (vec_t)ima.at(p + point2d{1, 0}) +
                         (vec_t)ima.at(p + point2d{1, 1})) /
                    2.0;
        ...
        if (MLN_HAS_DEBUG and curv.at(p_) > 100 and p > point2d{1, 1})
        {
          ...
          std::cout << norm((vec_t)ima.at(p + point2d{0, -1}) - (vec_t)ima.at(p) - (vec_t)ima.at(p + point2d{0, 1}) +
                            (vec_t)ima.at(p + point2d{0, 2}))
                    << std::endl;
      ...
      // Bottom edge
      {
        ...
        float uxx = norm((vec_t)ima.at(p + point2d{-1, 0}) - (vec_t)ima.at(p) - (vec_t)ima.at(p + point2d{1, 0}) +
                         (vec_t)ima.at(p + point2d{2, 0})) /
                    2.0;

        float uyy = norm((vec_t)ima.at(p + point2d{0, -1}) + (vec_t)ima.at(p + point2d{1, -1}) -
                         2 * ((vec_t)ima.at(p) + (vec_t)ima.at(p + point2d{1, 0})) + (vec_t)ima.at(p + point2d{0, 1}) +
                         (vec_t)ima.at(p + point2d{1, 1}) / 2.0);
  ...
  template <typename V, typename T>
  image2d<float> compute_energy(const image2d<V>& ima, const image2d<T>& K, const image2d<unsigned>& parent_,
