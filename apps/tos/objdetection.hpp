...
namespace mln
{
  ...
  inline unsigned zfindroot(image2d<unsigned>& par, unsigned x)
  {
    if (par[x] != x)
  ...
  template <typename V>
  void getObjects(image2d<float>& energy, image2d<V>& K, image2d<unsigned>& parent, const std::vector<unsigned>& S,
                  unsigned seuil)
  {
