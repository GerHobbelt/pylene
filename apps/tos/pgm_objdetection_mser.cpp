...
void usage(char** argv)
{
  std::cout << "Usage: " << argv[0] << " input(gray) output[wo ext] ε grain [λ₁ [λ₂...]]" << std::endl
            ...
            << "It creates as many images as lambdas named <output>-<λ>.tiff" << std::endl;
...
namespace mln
{
  ...
  template <typename V>
  image2d<unsigned> make_unique_id(const image2d<V>& K, const image2d<unsigned>& parent, const std::vector<unsigned>& S)
  {
...
int main(int argc, char** argv)
{
  ...
  for (int i = 5; i < argc; ++i)
  {
    ...
    int regions = compute_ellipse(cK, cParent, S, (boost::format("%s-%04f.mser") % argv[2] % lambda).str(), extmap,
                                  K1::is_face_2, [](point2d p) -> point2d {
                                    return {p[0] / 2, p[1] / 2};
                                  });
