...
namespace mln
{
  ...
  float coveryRate(const image2d<L>& labels, int nlabels, const image2d<V>& K, const image2d<unsigned>& parent,
      ...
      for (unsigned x : S)
      {
        if (K[x] != K[parent[x]] or x == parent[x])
        {
          ...
          for (int i = 0; i <= nlabels; ++i)
          {
            ...
            // std::cout << "\t Region: " << i << " size: " << count[i] << " inter: " << inter[x][i] << " cov: " << cov
            // << std::endl;
          }
        }
      }
