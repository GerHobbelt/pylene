...
usage(int argc, char** argv)
{
      ...
      (argv[2] != std::string("min") && argv[2] != std::string("max")))
  {
    std::cerr << "Usage: " << argv[0]
              << "(mintree|tos) (min|max) ima.(ppm|png|tiff...) out_without_extension grain mu1 [mu2  [mu3 [...]]]"
              << std::endl
              << "Compute the ToS marginally -> area -> a Tree and run simplification." << std::endl;
    abort();
  }
}
...
namespace mln
{
  ...
  image2d<V> setmean_on_nodes(const image2d<V>& ima, const image2d<T>& K, const image2d<unsigned>& parent,
  ...
  template <typename V, typename T, class FilterFun>
  void set_mean_on_01face(const image2d<T>& K, const image2d<unsigned>& parent, const std::vector<unsigned>& S,
                          image2d<V>& out, FilterFun filter)
  {
    struct myattribute
    {
      unsigned count;
  ...
  inline unsigned findcanonical(const image2d<bool>& is_removed, image2d<unsigned>& parent, unsigned p)
  {
    if (!is_removed[p])
      return p;
    else
  ...
  template <typename T>
  void simplify(image2d<T>& K, image2d<unsigned>& parent, const std::vector<unsigned>& S, image2d<attr_2f>& acc2,
                image2d<attr_1f>& acc1, float mu)
  {
  ...
  template <typename V>
  void close(image2d<V>& K, image2d<unsigned>& parent, const std::vector<unsigned>& S, unsigned lambda)
  {
    image2d<unsigned> count;
    resize(count, K).init(0);
