...
int main(int argc, const char** argv)
{
  ...
  io::imsave(transform(f, [sat](lsh8 x) -> bool { return x[1] > sat; }),
             (boost::format("%s-mask.tiff") % output).str().c_str());
  io::imsave(transform(f, [sat](lsh8 x) -> uint8 { return x[1]; }),
             (boost::format("%s-sat.tiff") % output).str().c_str());
  io::imsave(transform(f, [sat](lsh8 x) -> uint8 { return x[1] < sat ? x[0] : 0; }),
             (boost::format("%s-lum.tiff") % output).str().c_str());
  io::imsave(transform(f, [sat](lsh8 x) -> uint8 { return x[1] > sat ? x[2] : 0; }),
             (boost::format("%s-hue.tiff") % output).str().c_str());
  ...
  graphcut::graphcut(f, mask_, c4,
                     [sat](bool x, const lsh8& v) -> float {
                       return 0 * (x != (v[1] > sat)) + 1 * std::abs(x - 1.0 / (1.0 + std::exp(-(v[1] - sat) / 3.0)));
                     },
