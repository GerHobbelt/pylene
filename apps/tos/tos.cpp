...
po::variables_map usage(int argc, char** argv)
{
  ...
  desc.add_options()("help", "produce help message")("close", po::value<int>(), "Perform a closing.");

  po::options_description hidden("Allowed options");
  hidden.add_options()("tree", po::value<std::string>()->required(), "Type of tree (mintree|tos).")(
      "merge", po::value<std::string>()->required(),
      "Merge policy (min|max).")("input", po::value<std::string>()->required(), "Input color file (ppm|png...)")(
  ...
  catch (...)
  {
    std::cout << "Usage: " << argv[0]
              << " [options] (tos|mintree) (min|max|comp|grad) input.(tiff|ppm...) out(without extension) [lambda1 "
                 "[lambda2[...]]]"
              << std::endl
