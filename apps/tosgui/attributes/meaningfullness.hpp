...
public:
  ...
  virtual ~MeaningFullnessAttribute()
  {
    if (m_attribute_1)
    {
  ...
  virtual QMap<QString, Parameter>& parameters() { return m_params; }
...
protected slots:
  ...
  virtual void showinfo(const mln::point2d& node)
  {
    ...
    auto c3 =
        (m_acc(node).m_v_sum_ext + m_acc(node).m_v_sum_int) / (float)(m_acc(node).m_v_n_ext + m_acc(node).m_v_n_int);
...
public:
  virtual void run()
  {
    ...
    if (m_attribute_1 == NULL)
    {
      ...
      m_attribute_1 =
          new mln::QAttribute<float>(m_con, parent, QString("Meaningfullness - Constraint: exp(-gamma.area)"));
      m_attribute_2 = new mln::QAttribute<float>(m_curv, parent, QString("Meaningfullness - Curvature"));
      m_attribute_3 =
          new mln::QAttribute<float>(m_ext, parent, QString("Meaningfullness - (VarExt + VarInt) / VarTot"));
      ...
      QObject::connect(m_attribute_3, SIGNAL(nodeSelected(const mln::point2d&)), this,
                       SLOT(showinfo(const mln::point2d&)));
    ...
    if (eps != m_eps) // recompute: eps has changed
    {
      ...
      // Compute aux data
      {
        ...
        mln::transform(m_acc, [](const mln::internal::energy_t<T>& aux) { return aux.external_energy(); }, m_ext);
      }
    ...
    else
    {
      ...
      mln::transform(
          m_acc, [beta, gamma](const mln::internal::energy_t<T>& aux) { return beta * std::exp(-gamma * aux.m_area); },
          m_con);
      ...
      mln::transform(
          m_acc, [alpha, beta, gamma](const mln::internal::energy_t<T>& aux) { return aux.energy(alpha, beta, gamma); },
          m_energy);
      m_extinction = extinction(m_energy, K, parent, S, std::less<float>());
    }
  }

  virtual mln::QAttributeBase* getPlot(const QString& name)
  {
    static const char* names[] = {"MeaningFullness (Internal Energy Term - Variance)",
                                  "MeaningFullness (External Energy term - Curvature)",
                                  "MeaningFullness (Constraint Term - exp(-gamma.area) )",
                                  "MeaningFullness (All terms)", "MeaningFullness (Extinction)"};
