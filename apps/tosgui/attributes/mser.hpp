...
public:
  MSERAttribute(const mln::image2d<V>& K_, const mln::image2d<unsigned>& parent_, const std::vector<unsigned>& S_)
      : K(K_), parent(parent_), S(S_), m_param_h(-1), m_attribute_1(NULL), m_attribute_2(NULL), m_attribute_3(NULL),
        m_attribute_4(NULL)
  {
  ...
  virtual void run()
  {
    ...
    if (h != m_param_h)
    {
      ...
      m_cons = mln::transform(m_area, [a0, a1](float x) -> float { return a0 * std::exp(-x * a1); });
      m_mser_ = mln::eval(mln::transform(mln::imzip(m_mser, m_cons), [](const std::tuple<float, float>& v) {
