...
namespace mln
{
  ...
  void QDispatcher::addImageWindow(qt::ImageViewer* win)
  {
    m_windows.push_back(win);
    QObject::connect(win, SIGNAL(pointSelected(const mln::point2d&)), this, SLOT(onPointSelected(const mln::point2d&)));
  }

  void QDispatcher::addImageWindowToFilter(qt::ImageViewer* win, const image2d<rgb8>& mean)
  {
  ...
  void QDispatcher::setPlotWindow(PlotWindow* pltwin)
  {
    m_pltwin = pltwin;
    QObject::connect(pltwin, SIGNAL(nodeSelected(const mln::point2d&)), this,
                     SLOT(onNodeSelected(const mln::point2d&)));
    QObject::connect(pltwin, SIGNAL(nodeSelected(const mln::image2d<bool>&)), this,
                     SLOT(onNodeSelected(const mln::image2d<bool>&)));
  }

  void QDispatcher::onNodeSelected(const point2d& p)
  {
  ...
  void QDispatcher::onNodeSelected(const image2d<bool>& mask)
  {
    fill(m_mask_selection, false);
    int n = 0;
    for (unsigned p : m_S)
  ...
  void QDispatcher::onPointSelected(const point2d& p)
  {
    // Select leaf
    auto dist = [&p](const point2d& q) { return std::abs(p[0] - q[0]) + std::abs(p[1] - q[1]); };
  ...
  void QDispatcher::doNodeSection(qt::ImageViewer* win)
  {
