...
namespace mln
{
  ...
  {
  public:
    QAttribute(const image2d<V>& attr, const image2d<unsigned>& parent, const QwtText& name = QString("Attribute"));
  ...
  template <typename V>
  void QAttribute<V>::showFilteringWindow()
  {
    ...
    m_slider->show();
    std::cout << "Show filtering " << m_minmax.first << " / " << m_minmax.second << std::endl;

    QObject::connect(m_slider, SIGNAL(valueChanged(int)), this, SLOT(onSliderReleased(int)));
  }

  template <typename V>
  bool QAttribute<V>::eventFilter(QObject* obj, QEvent* event)
  {
  ...
  template <typename V>
  void QAttribute<V>::onSliderReleased(int)
  {
