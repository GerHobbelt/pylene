...
namespace mln
{
  ...
  std::vector<typename I::size_type> sort_indexes(const Image<I>& input, BinaryFunction cmp = BinaryFunction());

  template <typename I, typename OutputIterator, typename BinaryFunction = std::less<mln_value(I)>>
  void sort_indexes_it(const Image<I>& input, OutputIterator out, BinaryFunction cmp = BinaryFunction());
  ...
  namespace impl
  {
    ...
    template <typename I, typename StrictWeakOrdering, typename size_type = typename I::size_type,
              typename Indexer = indexer<mln_value(I), StrictWeakOrdering>>
    ...
    template <typename I, typename OutputIterator, typename StrictWeakOrdering,
              typename Indexer = indexer<mln_value(I), StrictWeakOrdering>>
    void sort_indexes(const I& input, OutputIterator v, StrictWeakOrdering cmp, use_std_sort_tag)
    {
      ...
      std::sort(v, v + input.domain().size(),
  ...
  std::vector<typename I::size_type> sort_indexes(const Image<I>& input, BinaryFunction cmp)
  {
    ...
    typedef typename std::conditional<(value_traits<mln_value(I)>::quant <= 18), impl::use_counting_sort_tag,
                                      impl::use_radix_sort_tag>::type dispatch_tag;
  ...
  void sort_indexes_it(const Image<I>& input, OutputIterator out, BinaryFunction cmp)
  {
    static_assert(std::is_same<typename image_category<I>::type, raw_image_tag>::value,
                  "Image must model the Raw Image Concept");
