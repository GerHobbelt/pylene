...
namespace mln
{
  ...
  template <class Domain, class V>
  struct constant_image : image_base<constant_image<Domain, V>, typename range_value<Domain>::type, V>
  {
  private:
...
#pragma warning(disable : 4521) // multiple copy constructors specified
    ...
    template <class I>
    struct pixel_iterator_t : iterator_base<pixel_iterator_t<I>, pixel_t<I>, pixel_t<I>>
    {
    private:
      typedef I image_type;
...
#pragma warning(pop)
    ...
    value_range values() const { return value_range(const_value_iterator(rng::iter(m_domain), m_value)); }

    value_range values() { return ((const constant_image*)this)->values(); }
    ...
    pixel_range pixels() { return pixel_range(pixel_iterator_t<constant_image>(this, rng::iter(m_domain), m_value)); }
    ...
    V operator()(const point_type& p) { return (*(const constant_image*)this)(p); }

    constexpr V at(const point_type&) const { return m_value; }

    V at(const point_type&) { return m_value; }

    pixel_type pixel(const point_type& p) { return pixel_type(this, p, m_value); }

    const_pixel_type pixel(const point_type& p) const { return const_pixel_type(this, p, m_value); }

    pixel_type pixel_at(const point_type& p) { return pixel_type(this, p, m_value); }

    const_pixel_type pixel_at(const point_type& p) const { return const_pixel_type(this, p, m_value); }

    constexpr V operator[](size_type) const { return m_value; }

    V operator[](size_type) { return m_value; }

    constexpr size_type index_of_point(const point_type&) const { return 0; }

    constexpr point_type point_at_index(size_type) const { return point_type(); }

    constexpr difference_type delta_index(const point_type&) const { return 0; }

    constexpr extension_type extension() const { return extension_type(); }
