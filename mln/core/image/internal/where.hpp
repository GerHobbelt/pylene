...
namespace mln
{
  ...
  template <typename I, class Predicate>
  internal::where_t<I, Predicate> where(Image<I>&& ima, const Predicate& pred);
  ...
  template <typename I>
  internal::where_binary_t<const I&> where(const Image<I>& ima);

  template <typename I>
  internal::where_binary_t<I> where(Image<I>&& ima);
  ...
  namespace internal
  {
    ...
    struct where_t<I, Predicate, std::true_type>
    {
      ...
      bool has(const mln_point(I) & p) const { return m_ima.get().domain().has(p) and m_pred(m_ima.get().pixel(p)); }

      const I& image() const { return m_ima.get(); }
    ...
    private:
      struct predicate_t
      {
        bool operator()(const mln_cpixel(I) & px) const { return pred(px.val()); }
    ...
    public:
      ...
      bool has(const mln_point(I) & p) const { return m_ima.get().domain().has(p) and m_pred.pred(m_ima.get()(p)); }
    ...
    {
    public:
      typedef transform_iterator<filter_iterator<mln_cpxter(I), meta_get_pixel_value>, meta_get_pixel_point> iterator;
      typedef iterator const_iterator;
      ...
      {
        return iterator(
            filter_iterator<mln_cpxter(I), meta_get_pixel_value>(m_ima.get().pixels().iter(), meta_get_pixel_value()),
            internal::meta_get_pixel_point());
      }

      bool has(const mln_point(I) & p) const { return m_ima.get().domain().has(p) and m_ima.get()(p); }
