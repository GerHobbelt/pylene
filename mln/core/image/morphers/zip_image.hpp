...
namespace mln
{
  ...
  template <class... Images>
  zip_image<Images...> imzip(Images&&... images);
  ...
  namespace internal
  {
    ...
    struct meta_get_value_range
    {
      ...
      template <typename I>
      mln_vrange(I) operator()(I& ima) const
      {
        return ima.values();
      }
    };
    ...
    struct meta_get_const_value_range
    {
      ...
      template <typename I>
      typename apply<I>::type operator()(const I& ima) const
      {
        return ima.values();
      }
    };
    ...
    struct meta_get_pixel_range
    {
      ...
      template <typename I>
      mln_pixrange(I) operator()(I& ima) const
      {
        return ima.pixels();
      }
    };
    ...
    struct meta_get_const_pixel_range
    {
      ...
      template <typename I>
      mln_cpixrange(I) operator()(const I& ima) const
      {
        return ima.pixels();
      }
    };
    ...
    struct meta_access_value
    {
      ...
      template <typename I>
      mln_reference(I) operator()(I& ima) const
      {
        return ima(m_p);
      }
    ...
    struct meta_const_access_value
    {
      ...
      template <typename I>
      mln_creference(I) operator()(const I& ima) const
      {
        return ima(m_p);
      }
    ...
    struct meta_get_pixval
    {
      ...
      template <typename Pix>
      typename Pix::reference operator()(const Pix& px) const
      {
        return px.val();
      }
    };
  ...
  public:
    ...
    const images_type& images() const { return m_images; }
    ...
    reference at(const point_type& p) { return internal::tuple_transform(m_images, internal::meta_at<point_type>{p}); }
    ...
    reference operator[](typename image_t::size_type i)
    {
      return internal::tuple_transform(m_images, internal::meta_access_index<typename image_t::size_type>{i});
    }

    template <typename image_t = I0>
    const_reference operator[](typename image_t::size_type i) const
    {
      return internal::tuple_transform(m_images, internal::meta_const_access_index<typename image_t::size_type>{i});
    }

  private:
    const I0& get_morphed() const { return std::get<0>(m_images); }

    I0& get_morphed() { return std::get<0>(m_images); }
  ...
  public:
    ...
    point_type point() const { return std::get<0>(m_pix_tuple).point(); }

    point_type site() const { return std::get<0>(m_pix_tuple).site(); }

    image_type& image() const { return *m_ima; }

    template <typename image_t = image_type>
    typename std::enable_if<image_traits<image_t>::indexable::value, typename image_t::size_type>::type index() const
    {
