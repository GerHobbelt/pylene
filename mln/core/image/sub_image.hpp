...
namespace mln
{
  ...
  sub_image<const I&, Domain> make_subimage(const Image<I>& ima, const Domain& domain,
                                            typename std::enable_if<not is_a<Domain, Image>::value>::type* = NULL);

  template <typename I, typename Domain>
  sub_image<I&, Domain> make_subimage(Image<I>& ima, const Domain& domain,
                                      typename std::enable_if<not is_a<Domain, Image>::value>::type* = NULL);
  ...
  auto operator|(const Image<I>& ima, const DomainOrMask& other) -> decltype(make_subimage(exact(ima), other));

  template <typename I, typename DomainOrMask>
  auto operator|(Image<I>& ima, const DomainOrMask& other) -> decltype(make_subimage(exact(ima), other));

  template <typename I, typename DomainOrMask>
  auto operator|(const Image<I>&& ima, const DomainOrMask& other) -> decltype(make_subimage(move_exact(ima), other));
  ...
  template <typename I, typename Domain>
  sub_image<const I&, Domain> make_subimage(const Image<I>& ima, const Domain& domain,
                                            typename std::enable_if<not is_a<Domain, Image>::value>::type*)
  {
  ...
  template <typename I, typename Domain>
  struct sub_image : image_base<sub_image<I, Domain>, typename range_value<Domain>::type,
  ...
  private:
    ...
    typedef rebind_pixel_iterator<self_t, transform_iterator<typename Domain::iterator, pix_fun_t>> pixel_iterator;
    typedef rebind_pixel_iterator<const self_t, transform_iterator<typename Domain::iterator, const_pix_fun_t>>
  ...
  public:
    ...
    pixel_range pixels()
    {
      ...
      return pixel_range(pixel_iterator(*this, make_transform_iterator(m_domain.iter(), pix_fun_t(m_ima))));
    }

    const_pixel_range pixels() const
    {
      return const_pixel_range(
          const_pixel_iterator(*this, make_transform_iterator(m_domain.iter(), const_pix_fun_t(m_ima))));
    }
    ...
    pixel_type pixel_at(const point_type& p) { return pixel_type(*this, m_ima.pixel_at(p)); }

    const_pixel_type pixel_at(const point_type& p) const { return const_pixel_type(*this, m_ima.pixel_at(p)); }
