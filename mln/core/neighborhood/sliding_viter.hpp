...
namespace mln
{
  ...
  template <class Pixel, class SiteSet>
  sliding_viter<Pixel, SiteSet>
  make_sliding_viter(const Pixel& pixel, const SiteSet& pset)
  {
    return {pixel, pset};
  }
  ...
  namespace internal
  {
    ...
    struct sliding_viter_base<
      ...
      typename std::enable_if< image_traits<image_t<Pixel>>::accessible::value and
			       ...
			       >::type>
      : iterator_base< sliding_viter<Pixel, SiteSet>,
                       typename image_t<Pixel>::value_type,
                       typename image_t<Pixel>::reference>
    {
    private:
	...
	: m_pixel (p),
      ...
      dereference() const
      {
        return m_pixel->image()(*m_pset_iter);
      }
    ...
    : iterator_base< sliding_viter<Pixel, std::array<P, N> >,
                     typename image_t<Pixel>::value_type,
                     typename image_t<Pixel>::reference>
    {
    private:
    ...
    public:
      sliding_viter_base() = default;
      sliding_viter_base(const Pixel& px, const std::array<P, N>& s)
	: m_pixel(px),
          m_i (0)
      {
	Image& ima = px->image();
	for (unsigned i = 0; i < N; ++i)
	  m_index_set[i] = ima.delta_index(s[i]);
      }
      ...
      dereference() const
      {
	return m_pixel->image()[m_index_set[m_i]];
      }
	...
	for (unsigned i = 0; i < m_size; ++i, it.next()) {
      ...
      dereference() const
      {
        ...
        return *(typename Image::value_type*)(current_ptr);
      }
	...
	Image& ima = px->image();
	for (unsigned i = 0; i < N; ++i) {
	  m_offset_set[i] = ima.delta_offset(s[i]);
	}
      }
...
namespace mln
{
  ...
  namespace internal
  {
    ...
    public:
      sliding_viter_base() = default;
      sliding_viter_base(const Pixel& px, const S& s)
	: m_site_set(s),
          m_pixel(px),
          m_i (0)
      {
