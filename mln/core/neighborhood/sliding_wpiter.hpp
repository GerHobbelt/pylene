...
namespace mln
{
  ...
  template <class Point, class SiteSet, class WeightSet>
  sliding_wpiter<Point, SiteSet, WeightSet>
  make_sliding_wpiter(const Point& p, const SiteSet& pset, const WeightSet& wset)
  {
    return {p, pset, wset};
  }
  ...
  public:
    ...
    void init() { m_pset_iter.init(); m_wset_iter.init(); }
    void next() { m_pset_iter.next(); m_wset_iter.next(); }
