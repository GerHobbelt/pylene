...
namespace mln
{
  ...
  template <typename Derived, typename Pix, typename Morpher = typename Pix::image_type>
...
#define MLN_PIXMORPHER_FORWARD_CONST_0(F, RETURN) MLN_PIXMORPHER_FORWARD_IF_0_(true, F, RETURN, const)
  ...
  namespace impl
  {
    ...
    template <typename Derived, typename Pix, typename Morpher,
              bool is_indexable = image_traits<Morpher>::indexable::value>
  ...
  public:
    ...
    pixel_t dereference() const { return pixel_t(*m_ima, *m_pixter); }
  ...
  struct get_pixel_value
  {
    ...
    struct apply
    {
      typedef typename Pix::reference type;
    };

    template <typename Pix>
    typename Pix::reference operator()(const Pix& pix) const
    {
      return pix.val();
    }
  };
  ...
  struct get_pixel_point
  {
    ...
    struct apply
    {
      typedef typename Pix::point_type type;
    };

    template <typename Pix>
    typename Pix::point_type operator()(const Pix& pix) const
    {
      return pix.point();
    }
  };
