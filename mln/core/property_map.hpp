...
namespace mln
{
  ...
  struct functional_property_map
  {
    ...
    template <class... T>
    typename std::result_of<F(T...)>::type operator[](T&&... args)
    {
      return f(std::forward<T>(args)...);
    }
  ...
  template <class F>
  functional_property_map<F, typename F::argument_type> make_functional_property_map(const F& fun)
  {
    return functional_property_map<F, typename F::argument_type>(fun);
  }

  template <class ArgType, class F>
  functional_property_map<F, ArgType> make_functional_property_map(const F& fun)
  {
