...
namespace mln
{
  ...
  namespace trace
  {
    ...
    inline scoped_trace::scoped_trace(const std::string& desc)
        : m_depth(__stack_depth++), m_desc(desc), m_clock(std::chrono::system_clock::now())
    {
      entering();
    }
    ...
    inline void scoped_trace::entering()
    {
      if (verbose)
      {
    ...
    inline void scoped_trace::exiting()
    {
      if (verbose)
      {
        for (int k = 0; k < m_depth; ++k)
        ...
        std::chrono::milliseconds duration =
            std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - m_clock);
