...
namespace mln
{
  ...
  namespace morpho
  {
    ...
    image2d<unsigned> area_compute(const image2d<V>& ima, const image2d<typename image2d<V>::size_type>& parent,
                                   const std::vector<typename image2d<V>::size_type>& S,
                                   const is_face_2_t& is_face2 = is_face_2_t(), const Compare& cmp = Compare());

    // template <typename T, typename V,  class is_face_2_t = internal::Yes, typename Compare = std::less<V>, typename
    // Sum > image2d< T > area_filter(const image2d< T >& ima, 		const image2d<V>& K, 		const
    // image2d<typename image2d<V>::size_type>& parent, 		const std::vector<typename
    // image2d<V>::size_type>& S, 		unsigned fvalue,
                ...
                const is_face_2_t& is_face2 = is_face_2_t(), const SumType& = SumType(), const Compare& cmp = Compare())
    {
      ...
      for (int i = S.size() - 1; i > 0; --i) // all except root
      {
        ...
        if (is_face2(K.point_at_index(p)))
        { // 2-face
          area[p] += 1;
          count[rp] += 1;
          acc[rp] += ima[p];
    ...
    image2d<rgb<T>> area_filterr(const image2d<rgb<T>>& ima, const image2d<V>& K,
