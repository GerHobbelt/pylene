...
namespace mln
{
  ...
  namespace morpgo
  {
	...
	maxtree_flood_algorithm(const I& ima, mln_ch_value(I, elt_type)& parent, const Neighborhood& nbh)
	  : m_ima(ima), m_parent(parent)
	{
	  initialize(m_deja_vu, ima, ima.border(), false);
	  m_nbh_delta_indexes = wrt_delta_index(ima, nbh);
	...
	decltype(wrt_delta_index(I (), Neighborhood ())) m_nbh_delta_indexes;
