...
namespace mln
{
  ...
  namespace qt
  {
    ...
    public:
      ImageViewerEventHandler(ImageViewer* viewer) : m_viewer(viewer), m_view_center(0, 0) {}
    ...
    void ImageViewerEventHandler::zoomBy(float factor)
    {
    ...
    void ImageViewerEventHandler::startZoomRect(QPointF wpos, QPointF scene_pos)
    {
      zooming.active = true;
    ...
    void ImageViewerEventHandler::endZoomRect(QPointF, QPointF scene_pos)
    {
    ...
    bool ImageViewerEventHandler::eventFilter(QObject* obj, QEvent* ev)
    {
    ...
    void ImageViewer::_init(QtImageBase* ima)
    {
      m_ima = ima;
      m_pixmap.setPixmap(QPixmap::fromImage(m_ima->getQImage()));

      m_scene = new QGraphicsScene(this);
      QObject::connect(m_scene, SIGNAL(sceneRectChanged(const QRectF&)), this, SLOT(onzoom(const QRectF&)));
    ...
    ImageViewer::~ImageViewer() { delete m_ima; }

    void ImageViewer::onzoom(const QRectF& rect)
    {
      int w = rect.right();
      int h = rect.bottom();
    ...
    void ImageViewer::reset() { m_ima->reset(); }

    image2d<rgb8>& ImageViewer::getView() { return m_ima->getView(); }

    const image2d<rgb8>& ImageViewer::getView() const { return m_ima->getView(); }

    QGraphicsScene* ImageViewer::getScene() { return m_scene; }

    const QGraphicsScene* ImageViewer::getScene() const { return m_scene; }

    QGraphicsPixmapItem* ImageViewer::getPixmap() { return &m_pixmap; }

    const QGraphicsPixmapItem* ImageViewer::getPixmap() const { return &m_pixmap; }
    ...
    bool ImageViewer::eventFilter(QObject* obj, QEvent* ev)
    {
      if (obj == m_scene)
      {
        ...
        else if (ev->type() == QEvent::GraphicsSceneMousePress)
        {
          ...
          if (m_ima->getView().domain().has(p))
          {
            std::cout << "Press: " << p << "," << (point2df{p__.x(), p__.y()}) << std::endl;
