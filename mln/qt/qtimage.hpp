...
namespace mln
{
  namespace qt
  {
    ...
    namespace internal
    {
      ...
      template <>
      inline void rgb_convert<uint8>(const image2d<uint8>& ima, image2d<rgb8>& out)
      {
        transform(ima, [](const uint8& x) { return rgb8{x, x, x}; }, out);
      }

      template <>
      inline void rgb_convert<rgb8>(const image2d<rgb8>& ima, image2d<rgb8>& out)
      {
        copy(ima, out);
      }
    }
