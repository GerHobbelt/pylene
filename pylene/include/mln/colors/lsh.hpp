...
namespace mln
{
  ...
  {
    template <typename T>
    inline std::tuple<T, T, T, int> get_MinMedMax(T r, T g, T b)
    {
      if (r < g)
      {
        if (g < b)
          return std::make_tuple(r, g, b, 3); // r < g < b
        else if (b < r)
          return std::make_tuple(b, r, g, 1); // b < r < g
        else
          return std::make_tuple(r, b, g, 2); // r < b <= g ou r <= b < g
      }
      else
      { // g <= r
        if (b < g)
          return std::make_tuple(b, g, r, 0); // b < g <= r
        else if (r < b)
          return std::make_tuple(g, r, b, 4); // g <= r < b
        else
  ...
  template <typename T>
  inline rgb<T> lsh2rgb(const lsh<T>& v)
  {
    static constexpr float k = (1 << value_traits<T>::quant) / 6;
    ...
    {
    case 0:
      return rgb<T>{max, med, min};
    case 1:
      return rgb<T>{med, max, min};
    case 2:
      return rgb<T>{min, max, med};
    case 3:
      return rgb<T>{min, med, max};
    case 4:
      return rgb<T>{med, min, max};
    case 5:
      return rgb<T>{max, min, med};
    default:
      assert(false);
      return rgb<T>();
    }
