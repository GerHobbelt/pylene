...
namespace mln::contrib::segdet
{
  ...
  struct Segment
  {
    ...
    std::vector<Point> under_other_object;
    ...
    Segment(std::vector<Point> points_vector, std::vector<Point>& underOther, float firstPartSlope,
            float lastPartSlope)
        : first_point(points_vector[0])
        , last_point(points_vector[points_vector.size() - 1])
        , points(std::move(points_vector))
        , under_other_object(std::move(underOther))
