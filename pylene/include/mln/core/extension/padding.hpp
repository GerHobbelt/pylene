...
namespace mln
{
  ...
  /// \brief Copy and pad a region of a ndimage or a mdspan in-place using one of the padding strategy
  ///
  /// Given `[0 0 1 2 3 0 0 0 0]`, borders = `{2, 4}` and value = `9`
  ///
  /// PAD_ZERO
  ...
  /// \param value The value uses when mode=PAD_CONSTANT
  //template <class T, std::size_t dim>
  //void copy_pad(const ranges::mdspan<T, dim>& input, const ranges::mdspan<T, dim>& out, mln::ndbox<dim> roi, const int borders[][2],  e_padding_mode mode,  T value = {});
