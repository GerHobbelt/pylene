...
namespace mln::extension
{
  ...
  */
  ...
  public:
    ...
    template <class I, class SE>
    [[gnu::noinline]]
    void manage(I&& f, const experimental::StructuringElement<SE>& se_) const
    {
  ...
  public:
    ...
    [[gnu::noinline]]
    std::optional<mln::extended_by_value_view<std::decay_t<InputImage>>>
    manage(InputImage&& f, const experimental::StructuringElement<SE>& se_) const
    {
      ...
      if (!val)
        throw std::runtime_error("Trying to fill the border with a bad value type. Ensure that value type fits the image type.");

      if (!image_has_extension<I>::value || !image_extension_t<I>::support_fill::value)
      {
        mln::trace::warn("[Performance] The image has no extension or does not support filling.");
        return view::extend_with_value(f, *val);
      }
      else if (mln::extension::need_adjust(f, se))
      {
