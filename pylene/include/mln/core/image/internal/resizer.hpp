...
namespace mln
{
  ...
  internal::resizer<I, J> resize(Image<I>& ima, const Image<J>& other); //__attribute__((deprecated));
  ...
  namespace internal
  {
    ...
    namespace impl
    {
      ...
      template <class OutputImage, class InputDomain>
      ...
      public:
        ...
        typename std::enable_if<has_border, dummy>::type border(int b)
        {
          if (b > m_border)
            m_border = b;
          return *this;
      ...
      protected:
        ...
        typename std::enable_if<has_border, dummy>::type do_it()
        {
          if (not m_has_hook)
            return;
      ...
      protected:
        ...
        template <class dummy = void>
        typename std::enable_if<has_border, dummy>::type __init_border()
        {
          this->m_border = this->m_ref.border();
        }

        template <class dummy = void>
        typename std::enable_if<not has_border, dummy>::type __init_border()
        {
          this->m_border = -1;
        }
      };
    ...
    {
    private:
      typedef typename std::conditional<is_a<InputImageOrInputDomain, Image>::value,
                                        impl::resizer_by_image<OutputImage, InputImageOrInputDomain>,
