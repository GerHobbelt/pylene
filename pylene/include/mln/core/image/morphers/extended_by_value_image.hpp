...
namespace mln
{
  ...
  template <typename I>
  extended_by_value_image<const I&> extend_by_value(const Image<I>& ima, const mln_value(I) & v);

  template <typename I>
  extended_by_value_image<I&> extend_by_value(Image<I>& ima, const mln_value(I) & v);
  ...
  public:
    ...
    typedef extended_by_value_image_pixel_iterator<
        this_t, typename range_iterator<typename image_pixel_range<image_t>::type>::type>
    ...
    typename std::enable_if<image_traits<image_t>::accessible::value, reference>::type operator()(const point_type& p)
    {
    ...
    template <typename = void>
    typename std::enable_if<image_traits<image_t>::accessible::value, reference>::type at(const point_type& p)
    {
    ...
    pixel_range pixels() { return pixel_range(pixel_range_iterator(*this, m_ima.pixels().iter())); }

    extension_type extension() const { return extension_type(const_cast<value_type&>(m_val)); }
