...
namespace mln
{
  ...
  template <typename I, class Predicate>
  filtered_image<I, Predicate> imfilter(Image<I>&& ima, const Predicate& pred);

  template <typename I, class Predicate>
  filtered_image<const I&, Predicate> imfilter(const Image<I>& ima, const Predicate& pred);
  ...
  private:
    ...
    typedef transform_iterator<filter_iterator<typename image_pixel_iterator<image_t>::type, Predicate>,
                               get_pixel_value>
        value_iterator;

    typedef transform_iterator<filter_iterator<typename image_const_pixel_iterator<image_t>::type, Predicate>,
                               get_pixel_value>
        const_value_iterator;

    typedef rebind_pixel_iterator<self_t, filter_iterator<typename image_pixel_iterator<image_t>::type, Predicate>>
        pixel_iterator;

    typedef rebind_pixel_iterator<const self_t,
  ...
  public:
    ...
    const_value_range values() const
    {
      return make_iterator_range(
          const_value_iterator(make_filter_iterator(m_ima.pixels().iter(), m_pred), get_pixel_value()));
    }

    value_range values()
    {
      return make_iterator_range(
          value_iterator(make_filter_iterator(m_ima.pixels().iter(), m_pred), get_pixel_value()));
    }

    const_pixel_range pixels() const
    {
      return make_iterator_range(
          const_pixel_iterator(static_cast<const self_t&>(*this), make_filter_iterator(m_ima.pixels().iter(), m_pred)));
    }

    pixel_range pixels()
    {
      return make_iterator_range(
          pixel_iterator(static_cast<self_t&>(*this), make_filter_iterator(m_ima.pixels().iter(), m_pred)));
    }
    ...
    const_reference at(const point_type& p) const { return m_ima.at(p); }

    reference at(const point_type& p) { return m_ima.at(p); }
  ...
  private:
    ...
    typedef filter_iterator<typename image_value_iterator<image_t>::type, Predicate> value_iterator;

    typedef filter_iterator<typename image_const_value_iterator<image_t>::type, Predicate> const_value_iterator;
    ...
    struct pixel_predicate_t
    {
      ...
      bool operator()(const const_pixel_t& pix) { return m_pred(pix.val()); }

    private:
      Predicate m_pred;
    };

    typedef rebind_pixel_iterator<self_t,
                                  filter_iterator<typename image_pixel_iterator<image_t>::type, pixel_predicate_t>>
        pixel_iterator;

    typedef rebind_pixel_iterator<
        const self_t, filter_iterator<typename image_const_pixel_iterator<image_t>::type, pixel_predicate_t>>
        const_pixel_iterator;
  ...
  public:
    ...
    const_value_range values() const { return const_value_range(make_filter_iterator(m_ima.values().iter(), m_pred)); }

    value_range values() { return value_range(make_filter_iterator(m_ima.values().iter(), m_pred)); }

    const_pixel_range pixels() const
    {
      return const_pixel_range(const_pixel_iterator(
          static_cast<const self_t&>(*this), make_filter_iterator(m_ima.pixels().iter(), pixel_predicate_t(m_pred))));
    }

    pixel_range pixels()
    {
      return pixel_range(pixel_iterator(static_cast<self_t&>(*this),
