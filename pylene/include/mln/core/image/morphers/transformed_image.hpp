...
namespace mln
{
  ...
  template <typename I, class UnaryFunction>
  transformed_image<const I&, UnaryFunction> imtransform(const Image<I>& ima, const UnaryFunction& f);
    ...
    {                                                                                                                  \
      ...
      {                                                                                                                \
        ...
        {                                                                                                              \
          ...
          result_type operator()(I f) const { return result_type(std::forward<I>(f), f_type()); }                      \
        };                                                                                                             \
      };                                                                                                               \
    }                                                                                                                  \
  }
  ...
  {                                                                                                                    \
    ...
    {                                                                                                                  \
      ...
      struct NAME##_helper_t;                                                                                          \
                                                                                                                       ...
                                                                                                                       \
    template <typename I>                                                                                              \
    typename internal::NAME##_helper_t<mln_value(I)>::template apply<I&&>::result_type NAME(Image<I>&& ima)            \
    {                                                                                                                  \
    ...
    {                                                                                                                  \
      return typename internal::NAME##_helper_t<mln_value(I)>::template apply<I&>()(exact(ima));                       \
                                                                                                                       ...
                                                                                                                       \
    template <typename I>                                                                                              \
    typename internal::NAME##_helper_t<mln_value(I)>::template apply<const I&>::result_type NAME(const Image<I>& ima)  \
    {                                                                                                                  \
      return typename internal::NAME##_helper_t<mln_value(I)>::template apply<const I&>()(exact(ima));                 \
    }                                                                                                                  \
  }

#define MLN_DECLARE_IMAGE_LVALUE_OPERATOR_OVERLOAD(NAME, TYPE, F) MLN_INTERNAL_IMAGE_LVALUE_OPERATOR_2(NAME, TYPE, F)
...
namespace mln
{
  ...
  namespace internal
  {
    ...
    public:
      ...
      typedef
          typename std::conditional<std::is_rvalue_reference<_result_type>::value,
                                    typename std::remove_reference<_result_type>::type, _result_type>::type reference;
      ...
      typedef
          typename std::conditional<std::is_reference<reference>::value,
    ...
    public:
      ...
      {
        using mln::imchvalue;
        return imchvalue<value_type>(f.m_ima);
      }
      ...
      typename std::enable_if<image_traits<this_t>::accessible::value, dummy>::type operator()(const mln_point(I) & p)
      {
      ...
      typename std::enable_if<image_traits<this_t>::accessible::value, dummy>::type operator()(const mln_point(I) &
                                                                                               p) const
      {
        mln_precondition(this->domain().has(p));
        return m_fun(this->m_ima(p));
      ...
      template <typename dummy = reference>
      typename std::enable_if<image_traits<this_t>::accessible::value, dummy>::type at(const mln_point(I) & p)
      {
        return m_fun(this->m_ima.at(p));
      }

      template <typename dummy = const_reference>
      typename std::enable_if<image_traits<this_t>::accessible::value, dummy>::type at(const mln_point(I) & p) const
      {
        return m_fun(this->m_ima.at(p));
      }

      template <typename dummy = pixel_type>
      typename std::enable_if<image_traits<this_t>::accessible::value, dummy>::type pixel(const mln_point(I) & p)
      {
        mln_precondition(this->domain().has(p));
      ...
      typename std::enable_if<image_traits<this_t>::accessible::value, dummy>::type pixel(const mln_point(I) & p) const
      {
        mln_precondition(this->domain().has(p));
      ...
      template <typename dummy = pixel_type>
      typename std::enable_if<image_traits<this_t>::accessible::value, dummy>::type pixel_at(const mln_point(I) & p)
      {
        return pixel_type(this->m_ima.pixel_at(p), this);
      }

      template <typename dummy = const_pixel_type>
      typename std::enable_if<image_traits<this_t>::accessible::value, dummy>::type pixel_at(const mln_point(I) &
                                                                                             p) const
      {
