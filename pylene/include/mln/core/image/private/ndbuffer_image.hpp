...
namespace mln
{
  ...
  public:
    ...
    __ndbuffer_image(int width, const image_build_params& params = image_build_params()) requires(N == 1);
    __ndbuffer_image(int width, int height, const image_build_params& params = image_build_params()) requires(N == 2);
    __ndbuffer_image(int width, int height, int depth,
                     const image_build_params& params = image_build_params()) requires(N == 3);


    __ndbuffer_image(std::initializer_list<T> values) requires(N == 1);
    __ndbuffer_image(std::initializer_list<std::initializer_list<T>> values) requires(N == 2);
    __ndbuffer_image(std::initializer_list<std::initializer_list<std::initializer_list<T>>> values) requires(N == 3);
    ...
    void resize(int width, const image_build_params& params = image_build_params()) requires(N == 1);
    void resize(int width, int height, const image_build_params& params = image_build_params()) requires(N == 2);
  ...
  template <class T, int N>
  inline void __ndbuffer_image<T, N>::__init(alloc_fun_t __allocate, int topleft[], int sizes[],
                                             std::ptrdiff_t byte_strides[], const image_build_params& params)
  {
    base::__init(__allocate, sample_type_traits<T>::id(), sizeof(T), N, topleft, sizes, byte_strides, params);
  }

  template <class T, int N>
  inline auto __ndbuffer_image<T, N>::concretize() const noexcept
      -> image_builder<concrete_type, __ndbuffer_image<T, N>>
  {
    return {*this};
  }
  ...
  template <class T, int N>
  template <class V>
  inline auto __ndbuffer_image<T, N>::ch_value() const noexcept
      -> image_builder<ch_value_type<V>, __ndbuffer_image<T, N>>
  {
    return {*this};
  }
  ...
  template <class T, int N>
  inline __ndbuffer_image<T, N> __ndbuffer_image<T, N>::from_buffer(T* buffer, int sizes[],
                                                                    std::ptrdiff_t byte_strides[], bool copy)
  {
    int topleft[N] = {0};
  ...
  template <class T, int N>
  inline __ndbuffer_image<T, N>::__ndbuffer_image(int width, const image_build_params& params) requires(N == 1)
  {
  ...
  template <class T, int N>
  inline __ndbuffer_image<T, N>::__ndbuffer_image(int width, int height,
                                                  const image_build_params& params) requires(N == 2)
  {
    this->resize(width, height, params);
  }

  template <class T, int N>
  inline __ndbuffer_image<T, N>::__ndbuffer_image(int width, int height, int depth,
                                                  const image_build_params& params) requires(N == 3)
  {
  ...
  template <class T, int N>
  inline __ndbuffer_image<T, N>::__ndbuffer_image(std::initializer_list<T> values) requires(N == 1)
    : base(std::move(values))
  {
  }

  template <class T, int N>
  inline __ndbuffer_image<T, N>::__ndbuffer_image(std::initializer_list<std::initializer_list<T>> values) requires(N ==
                                                                                                                   2)
    : base(std::move(values))
  {
  }
  ...
  template <class T, int N>
  inline __ndbuffer_image<T, N>::__ndbuffer_image(
      std::initializer_list<std::initializer_list<std::initializer_list<T>>> values) requires(N == 3)
    : base(std::move(values))
  {
  }
  ...
  template <class T, int N>
  inline void __ndbuffer_image<T, N>::resize(int width, const image_build_params& params) requires(N == 1)
  {
  ...
  template <class T, int N>
  inline void __ndbuffer_image<T, N>::resize(int width, int height, const image_build_params& params) requires(N == 2)
  {
  ...
  template <class T, int N>
  inline void __ndbuffer_image<T, N>::resize(int width, int height, int depth,
                                             const image_build_params& params) requires(N == 3)
  {
