...
namespace mln
{
  ...
  public:
    ...
    using extension_type     = extension::by_clamp<image_value_t<I>, I>;
    ...
    struct new_pixel_type : pixel_adaptor<image_pixel_t<I>>, experimental::Pixel<new_pixel_type>
    {
      ...
      reference val() const
      {
        ...
        return m_dom.has(pnt) ? this->base().val() : m_ima->compute_clamped_value(pnt);
      }

      new_pixel_type(image_pixel_t<I> px, clamp_extended_view<I>* ima, domain_type dom)
  ...
  private:
    ...
    Coord compute_coords_impl(const Coord& pnt, std::size_t shp_) const
    {
      if (shp_ == 0)
