...
namespace mln
{
  ...
  namespace detail
  {
    constexpr bool have_compatible_dim(int d1, int d2)
    {
  ...
  private:
    template <int, class>
    friend struct impl::pref;
    template <int, class>
    friend struct impl::pcontainer;
    template <class U>
    friend class _point;
  ...
  public:
    ...
    _point(int dim) requires(Impl::ndim == -1 && has_value_semantic)
      : Impl(dim)
    {
    }
    ...
    template <class I2>
    constexpr _point(const _point<I2>& other) noexcept requires(detail::have_compatible_dim(ndim, I2::ndim))
      : Impl(other)
    {
    }
    ...
    */
    ...
    constexpr T at(int k) const noexcept
    {
      assert(k < dim());
      return this->data(k);
    }
    constexpr T& at(int k) noexcept
    {
      assert(k < dim());
      return this->data(k);
    }

    constexpr T operator[](int k) const noexcept
    {
      assert(k < dim());
      return this->data(k);
    }
    constexpr T& operator[](int k) noexcept
    {
      assert(k < dim());
      return this->data(k);
    }
    ...
    constexpr T y() const noexcept requires(Impl::ndim == -1 || Impl::ndim >= 2)
    {
      assert(dim() >= 2);
      return this->data(1);
    }

    constexpr T& y() noexcept requires(Impl::ndim == -1 || Impl::ndim >= 2)
    {
      assert(dim() >= 2);
      return this->data(1);
    }

    constexpr T z() const noexcept requires(Impl::ndim == -1 || Impl::ndim >= 3)
    {
      assert(dim() >= 3);
      return this->data(2);
    }

    constexpr T& z() noexcept requires(Impl::ndim == -1 || Impl::ndim >= 3)
    {
      assert(dim() >= 3);
      return this->data(2);
    }
  ...
  */
  ...
  namespace impl
  {
    ...
    struct pcontainer<-1, T>
    {
      ...
      template <class Impl>
      constexpr pcontainer(const _point<Impl>& other) noexcept
          requires(std::convertible_to<typename Impl::value_type, T>)
        : m_dim(other.dim())
      {
    ...
    struct pref
    {
      ...
      constexpr pref(T* data) noexcept
        : m_data(data)
      {
      }
      ...
      template <class _i>
      constexpr pref(const _point<_i>& p) noexcept requires(std::convertible_to<typename _i::value_type const*, T*>)
        : m_data{p.data()}
      {
        assert(p.dim() == Dim_);
      }
      ...
      template <class _i>
      constexpr pref(_point<_i>& p) noexcept requires(std::convertible_to<typename _i::value_type*, T*>)
        : m_data{p.data()}
      {
        assert(p.dim() == Dim_);
      }
    ...
    struct pref<-1, T>
    {
      ...
      constexpr pref(int dim, T* data) noexcept
        : m_dim(dim)
        , m_data(data)
      {
      }
  ...
  template <class Impl>
  auto operator-(_point<Impl> a) noexcept requires(Impl::ndim == -1)
  {
  ...
  template <class Impl, class U>
  auto operator*(U x, _point<Impl> a) noexcept requires(Impl::ndim != -1)
  {
  ...
  template <class Impl, class U>
  auto operator*(_point<Impl> a, U x) noexcept requires(Impl::ndim != -1)
  {
    using R = decltype(x * a[0]);
  ...
  template <class I1, class I2>
  auto operator+(_point<I1> a, _point<I2> b) noexcept requires(I1::ndim == -1 && I2::ndim == -1)
  {
