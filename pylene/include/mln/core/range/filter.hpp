...
namespace mln
{
  ...
  public:
    ...
    iterator iter() { return iterator(m_rng.iter(), m_pred); }

    bool has(const value_type& v) const { return m_pred(v) and m_rng.has(v); }
  ...
  namespace rng
  {
    ...
    template <typename InputRange, typename Predicate>
    filtered_range<InputRange, Predicate> filter(InputRange&& rng, Predicate pred)
    {
