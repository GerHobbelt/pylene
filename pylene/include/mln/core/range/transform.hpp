...
namespace mln
{
  ...
  public:
    ...
    const_iterator iter() const { return const_iterator(m_rng.iter(), m_fun); }

    iterator iter() { return iterator(m_rng.iter(), m_fun); }
  ...
  namespace rng
  {
    ...
    template <typename InputRange, typename UnaryFunction>
    transformed_range<InputRange, UnaryFunction> transform(InputRange&& rng, const UnaryFunction& fun)
    {
