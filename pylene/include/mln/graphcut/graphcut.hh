...
namespace mln
{
  ...
  namespace graphcut
  {
    ...
    template <typename V, class N, class DataFidelityFunction, class RegularityFunction,
              class Compare = internal::default_cmp<DataFidelityFunction, RegularityFunction, V>>
    ...
    namespace internal
    {
      ...
      public:
        ...
        struct edge_type
        {
          ...
          {
            return (this->type < other.type) or (this->type == other.type and this->p == other.p);
          }
        };
        ...
        struct edges_iterator : iterator_base<edges_iterator, edge_type, const edge_type&>
        {
          ...
          bool finished() const { return m_e.type == SINK_EDGE and m_domain_it.finished(); }
        ...
        const Etype& edge_at(const edge_type& e) const { return const_cast<graphcut_graph_t*>(this)->edge_at(e); }

        const Etype& edge(const edge_type& e) const { return const_cast<graphcut_graph_t*>(this)->edge(e); }
        ...
        vertices_range vertices() const { return vertices_range(vertices_iterator(base::vertices(), SOURCE, SINK)); }

        edges_range edges() const { return edges_range(edges_iterator(base::edges(), base::vertices())); }

        adjacency_vertex_range adjacent_vertices(const point2d& v) const
        {
          return adjacency_vertex_range(
              adjacency_vertex_iterator(base::adjacent_vertices(v).iter(), base::vertices(), SOURCE, SINK, v));
        }

        adjacency_edge_range edges(const point2d& v) const
        {
          return adjacency_edge_range(
              adjacency_edge_iterator(base::edges(v).iter(), base::vertices(), SOURCE, SINK, v));
        }
      ...
      template <typename G>
      bool is_orphan(G& graph, const point2d& p)
      {
          ...
          for (; not qit.finished(); qit.next())
          {
            ...
            if (cmp(tree_cap(p, q), 0) > 0)
            {
              ...
              if (zq == q and zq != graph.SOURCE and zq != graph.SINK) // q is a free node, add it as an an active node
              {
                ...
                if (!inqueue)
                  active.push(q);
                // std::cout << "  Push " << q << "(par,zpar,cap: " << p << "," << zp << "," << tree_cap(p,q) << ") in
                // active list." << std::endl;
              }
            ...
            if (!adopted)
            {
              mln_forall (qit)
              {
                ...
                if (x.zpar == y.zpar)
                {
                  ...
                  if (y.par == p)
                  { // q points to an orphan so is an orphan itself
