...
namespace mln
{
  ...
  namespace io
  {
    ...
    private:
      ...
      template <class colortype, class FI_type>
      void read_next_line_rgba(void* out);
      template <class colortype, class FI_type>
      void read_next_pixel_rgba(void* out);
    ...
    inline int freeimage_reader_plugin::get_bpp() const { return bpp; }

    inline std::function<void(void*)> freeimage_reader_plugin::get_read_next_line_method() const
    {
      return m_read_next_line;
    }

    inline std::function<void(void*)> freeimage_reader_plugin::get_read_next_pixel_method() const
    {
      return m_read_next_pixel;
    }

    inline void freeimage_reader_plugin::open(std::istream& is)
    {
    ...
    inline void freeimage_reader_plugin::_load()
    {
      mln_precondition(m_dib != NULL);
      m_domain.pmin[0] = 0;
      m_domain.pmin[1] = 0;
      ...
      if (type == FIT_BITMAP)
      {
        ...
        case 1:
          ...
          {
            m_read_next_line =
                std::bind(&freeimage_reader_plugin::read_next_line_bool_inv, this, std::placeholders::_1);
            m_read_next_pixel =
                std::bind(&freeimage_reader_plugin::read_next_pixel_bool_inv, this, std::placeholders::_1);
          }
          break;
        case 8:
          if (colortype == FIC_MINISBLACK)
          ...
          {
            m_read_next_line =
                std::bind(&freeimage_reader_plugin::read_next_line_gray_palette, this, std::placeholders::_1);
            m_read_next_pixel =
                std::bind(&freeimage_reader_plugin::read_next_pixel_gray_palette, this, std::placeholders::_1);
            m_vtype = typeid(rgb8);
            break;
          }
          goto error;
    ...
    inline std::type_index freeimage_reader_plugin::get_value_type_id() const
    {
      mln_precondition(m_dib != NULL);
      return m_vtype;
    }
    ...
    inline void freeimage_reader_plugin::read_next_line_bool(void* out_)
    {
      mln_precondition(m_dib != NULL);
    ...
    inline void freeimage_reader_plugin::read_next_pixel_bool(void* out_)
    {
      mln_precondition(m_dib != NULL);
    ...
    inline void freeimage_reader_plugin::read_next_line_bool_inv(void* out_)
    {
      mln_precondition(m_dib != NULL);
    ...
    inline void freeimage_reader_plugin::read_next_pixel_bool_inv(void* out_)
    {
      mln_precondition(m_dib != NULL);
    ...
    inline void freeimage_reader_plugin::read_next_line_gray(void* out)
    {
      std::memcpy(out, m_ptr, m_domain.pmax[1] * bpp);
      m_ptr -= pitch;
    }

    inline void freeimage_reader_plugin::read_next_pixel_gray(void* out)
    {
    ...
    inline void freeimage_reader_plugin::read_next_line_rgb8(void* out)
    {
      rgb8* buffer = (rgb8*)out;
    ...
    inline void freeimage_reader_plugin::read_next_pixel_rgb8(void* out)
    {
      rgb8* pixel = (rgb8*)out;
    ...
    inline void freeimage_reader_plugin::read_next_line_rgba8(void* out)
    {
    ...
    inline void freeimage_reader_plugin::read_next_line(void* out_)
    {
      mln_precondition(m_dib != NULL);
      m_read_next_line(out_);
    }

    inline void freeimage_reader_plugin::read_next_pixel(void* out_)
    {
      mln_precondition(m_dib != NULL);
    ...
    inline std::function<void(void*)> freeimage_writer_plugin::get_write_next_line_method() const
    {
      return m_write_next_line;
    }

    inline std::function<void(void*)> freeimage_writer_plugin::get_write_next_pixel_method() const
    {
      return m_write_next_pixel;
    }

    inline void freeimage_writer_plugin::_get_fit_and_bpp_from_type(FREE_IMAGE_TYPE& fit, unsigned& bpp,
                                                                    std::type_index vtype) const
    {
      static std::unordered_map<std::type_index, std::pair<FREE_IMAGE_TYPE, int>> tinfo = {
          {typeid(bool), {FIT_BITMAP, 1}},      {typeid(uint8), {FIT_BITMAP, 8}},
          {typeid(rgb8), {FIT_BITMAP, 24}},     {typeid(colors::rgba8), {FIT_BITMAP, 32}},
          {typeid(uint16), {FIT_UINT16, 16}},   {typeid(int16), {FIT_INT16, 16}},
          {typeid(uint32), {FIT_UINT32, 32}},   {typeid(int32), {FIT_INT32, 32}},
          {typeid(float), {FIT_FLOAT, 32}},     {typeid(double), {FIT_DOUBLE, 64}},
          {typeid(rgb16), {FIT_RGB16, 48}},     {typeid(colors::rgba16), {FIT_RGBA16, 64}},
          {typeid(rgb<float>), {FIT_RGBF, 96}}, {typeid(colors::rgba<float>), {FIT_RGBAF, 128}}};
    ...
    inline void freeimage_writer_plugin::set_value_type(std::type_index type)
    {
      m_vtype = type;
      if (m_ncols > 0 and m_nrows > 0 and m_vtype != typeid(void))
        this->_allocate();
    ...
    inline freeimage_writer_plugin::~freeimage_writer_plugin()
    {
      FreeImageIO fio = {NULL, internal::ostream_wrapper::write, internal::ostream_wrapper::seek,
    ...
    // 1-bit DIBs are stored using each bit as an index into the color table. The most
    // significant bit is the leftmost pixel.
    inline void freeimage_writer_plugin::write_next_line_bool(void* src_)
    {
      mln_precondition(m_dib != NULL);
    ...
    inline void freeimage_writer_plugin::write_next_pixel_bool(void* src_)
    {
      mln_precondition(m_dib != NULL);
    ...
    // Non standard image types such as short, long, float or double do not have a color
    // table. Pixels are stored in a similar way as 8-bit DIB
    inline void freeimage_writer_plugin::write_next_line_gray(void* src)
    {
      std::memcpy(m_ptr, src, byteperline);
      m_ptr -= pitch;
    }

    inline void freeimage_writer_plugin::write_next_pixel_gray(void* src)
    {
    ...
    // 24-bit DIBs have every 3 bytes representing a color, using the
    // same ordering as the RGBTRIPLE structure.
    inline void freeimage_writer_plugin::write_next_line_rgb8(void* src)
    {
    ...
    inline void freeimage_writer_plugin::write_next_line_rgba8(void* src)
    {
    ...
    // Main method that set the right
    inline void freeimage_writer_plugin::write_next_line(void* out_)
    {
      mln_precondition(m_dib != NULL);
      m_write_next_line(out_);
    }

    inline void freeimage_writer_plugin::write_next_pixel(void* out_)
    {
      mln_precondition(m_dib != NULL);
