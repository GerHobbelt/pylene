...
namespace mln
{
  ...
  namespace io
  {
    ...
    {
      static_assert(std::is_same<typename I::domain_type, box2d>::value, "The domain must be a box 2D.");
    ...
    template <class I>
    void Loader<I>::load(const std::string& s, Image<I>& ima, PluginReader* plugin, bool permissive)
    {
...
#define MLN_INTERNAL_SCALAR_TYPE_SEQ (bool)(int8)(uint8)(int16)(uint16)(int32)(uint32)(int64)(uint64)(float)(double)
...
#define MLN_INTERNAL_CONV_EXPAND(r, data, VIN)                                                                         \
  if (sidx == typeid(VIN))                                                                                             \
    return default_converter<VIN, VOUT>::ptr;

      template <typename VOUT>
      std::function<void(void*, void*, std::size_t n)> defaut_value_type_converter(std::type_index sidx)
      {
    ...
    template <class I>
    inline bool Loader<I>::is_value_type_convertible_from(std::type_index sidx) const
    {
    ...
    template <class I>
    void Loader<I>::m_check_value_type_compatible() const
    {
      typedef mln_value(I) V;
      std::type_index sidx = m_plugin->get_value_type_id();
      std::type_index tidx = typeid(V);

      if (sidx != tidx and (not m_permissive or not is_value_type_convertible_from(sidx)))
      {
    ...
    void Loader<I>::m_load()
    {
      ...
      std::function<void(void*)> read_next_pixel = m_plugin->get_read_next_pixel_method();
