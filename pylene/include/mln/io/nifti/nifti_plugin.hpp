...
namespace mln
{
  ...
  namespace io
  {
    ...
    inline void nifti_reader_plugin::open(std::istream&)
    {
      throw MLNIOException("NIFTI does not support istreams as input.");
    }

    inline void nifti_reader_plugin::open(const char* filename)
    {
      // Only read header
    ...
    inline box2d nifti_reader_plugin::get_domain() const { return m_domain; }

    inline int nifti_reader_plugin::get_bpp() const { return m_bpp; }

    inline std::type_index nifti_reader_plugin::get_value_type_id() const { return m_vtype; }

    inline std::function<void(void*)> nifti_reader_plugin::get_read_next_line_method() const
    {
      return m_read_next_line;
    }

    inline std::function<void(void*)> nifti_reader_plugin::get_read_next_pixel_method() const
    {
      return m_read_next_pixel;
    }

    inline void nifti_reader_plugin::read_next_line_(void* out)
    {
      std::memcpy(out, m_ptr, m_domain.pmax[1] * m_bpp);
      m_ptr -= m_stride;
    }

    inline void nifti_reader_plugin::read_next_pixel_(void* out)
    {
