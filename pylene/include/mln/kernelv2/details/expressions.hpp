...
namespace mln
{
  namespace kernel
  {
    namespace details
    {
      ...
      using image_call_p_expr = proto::expr<tag::image_call_p<k>, proto::term<I>>;

      template <class I, int k>
      using image_call_n_expr = proto::expr<tag::image_call_n<k>, proto::term<I>>;

      template <class Feature, class Expr>
      using aggregate_expr = typename proto::result_of::make_expr<
          kernel::tag::aggregate,
          typename mln::accu::accu_of<Feature, typename std::decay<typename proto::result_of::eval<
