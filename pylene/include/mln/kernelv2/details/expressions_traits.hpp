...
namespace mln
{
  namespace kernel
  {
    namespace details
    {
      ...
      template <class Expr, int i, class Tag = typename proto::tag_of<Expr>::type>
      ...
      template <class Expr>
      typename image_list_traits<Expr>::type get_image_list(Expr&& x);

      template <class Expr>
      typename image_list_traits<Expr>::zip_image_type get_image_list2(Expr&& x);
      ...
      template <class Expr>
      typename image_used_by_neighbor_list_traits<Expr>::type get_image_used_by_neighbor_list(Expr&& x);

      template <class Expr>
      typename image_used_by_neighbor_list_traits<Expr>::zip_image_type get_image_used_by_neighbor_list2(Expr&& x);

      template <class I>
      using null_image_t = constant_image<typename std::decay<I>::type::domain_type, dontcare_t>;
      ...
      {
        typedef
            typename mpl::deref<typename mpl::max_element<mpl::list<typename ChildTrait::number_of_images...>>::type>::
      ...
      {
        typedef typename mpl::fold<mpl::list<typename ChildTrait::type...>, mpl::void_,
                                   mpl::if_<boost::is_same<mpl::_1, mpl::void_>, mpl::_2, mpl::_1>>::type type;

        typedef
            typename mpl::fold<mpl::list<typename ChildTrait::point...>, mpl::false_, mpl::or_<mpl::_1, mpl::_2>>::type
                point;

        typedef typename mpl::fold<mpl::list<typename ChildTrait::neighbor...>, mpl::false_,
      ...
      template <class Expr, class Seq = typename int_list_seq<number_of_images<Expr>::value>::type>
      ...
      template <class Expr, class Ses = typename int_list_seq<number_of_images<Expr>::value>::type>
      ...
      struct image_used_by_neighbor_list_traits_base<Expr, intseq<k...>>
      {
        ...
        typedef std::tuple<
            typename std::conditional<image_usage_traits<Expr, k>::neighbor::value,
                                      typename image_usage_traits<Expr, k>::type, null_image_type>::type...>
            type;
      ...
      typename image_usage_traits<Expr, k>::type get_image(Expr&& x)
      {
        return image_getter<k>()(std::forward<Expr>(x)).get(); //  proto::value(*res);
      }

      template <class Expr, int... k>
      typename image_list_traits<Expr>::type get_image_list_helper(Expr&& x, intseq<k...>)
      {
        return std::forward_as_tuple(image_getter<k>()(std::forward<Expr>(x)).get()...);
      }

      template <class Expr, int... k>
      typename image_list_traits<Expr>::zip_image_type get_image_list_helper2(Expr&& x, intseq<k...>)
      {
      ...
      template <class Expr, int... k>
      typename image_used_by_neighbor_list_traits<Expr>::type get_image_used_by_neighbor_list_helper(Expr&& x,
                                                                                                     intseq<k...>)
      {
