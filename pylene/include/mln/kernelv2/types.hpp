...
namespace mln
{
  namespace kernel
  {
    ...
    struct Image
    {
      ...
      details::image_call_p_expr<std::reference_wrapper<T>, k> operator()(Point)
      {
        return details::image_call_p_expr<std::reference_wrapper<T>, k>{f};
      }

      details::image_call_n_expr<std::reference_wrapper<T>, k> operator()(Neighbor)
      {
    ...
    struct Aggregate
    {
      ...
      details::aggregate_expr<Feature, Expr> operator()(Expr&& expr) const
      {
        ...
        return proto::make_expr<tag::aggregate, typename accu::accu_of<Feature, V>::type, Expr>(
            accu::make_accumulator(feature, V()), std::forward<Expr>(expr));
      }
    ...
    Image<I, k> make_image_expr(I&& f)
    {
      static_assert(is_a<I, mln::Image>::value, "The first parameter is not an image.");
