...
namespace mln
{
  ...
  namespace morpho
  {
    ...
    mln_concrete(I)
        area_closing(const Image<I>& ima, const Neighborhood<N>& nbh, unsigned area, Compare cmp = Compare());
    ...
    mln_concrete(I) area_opening(const Image<I>& ima, const Neighborhood<N>& nbh, unsigned area);
    ...
    namespace internal
    {
      ...
      {
        void on_make_set(const mln_point(I) & p) { m_acc_img(p).take(p); }

        void on_union(const mln_point(I) & p, const mln_point(I) & q) { m_acc_img(q).take(m_acc_img(p)); }

        void on_finish(const mln_point(I) & p, const mln_point(I) & q) { m_rec(p) = m_rec(q); }
    ...
    mln_concrete(I) area_closing(const Image<I>& ima_, const Neighborhood<N>& nbh, unsigned area, Compare cmp)
    {
      ...
      mln::morpho::canvas::unionfind(
          ima, nbh, [&accu_img, area](const mln_point(I) & p) { return accu_img(p).to_result() >= area; }, cmp, viz);
      mln_exiting();
      return out;
    }

    template <class I, class N>
    mln_concrete(I) area_opening(const Image<I>& ima_, const Neighborhood<N>& nbh, unsigned area)
    {
      mln_entering("mln::morpho::area_opening");
      mln_concrete(I) out = area_closing(exact(ima_), exact(nbh), area, std::greater<mln_value(I)>());
      mln_exiting();
      return out;
    }
  } // namespace morpho
