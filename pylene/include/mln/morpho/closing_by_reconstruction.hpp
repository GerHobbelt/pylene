...
namespace mln
{
  ...
  namespace morpho
  {
    ...
    mln_concrete(I) closing_by_reconstruction(const Image<I>& f, const Image<J>& markers, const Neighborhood<N>& nbh,
                                              Compare cmp = Compare());
    ...
    namespace internal
    {
      ...
      struct closing_by_rec_ufind_visitor
      {
        ...
        void on_make_set(const mln_point(I) & p) { m_accu(p).take(m_markers(p)); }

        void on_union(const mln_point(I) & p, const mln_point(I) & q) { m_accu(q).take(m_accu(p)); }

        void on_finish(const mln_point(I) & p, mln_point(I) & q) { m_out(p) = m_out(q); }
