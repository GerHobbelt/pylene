...
namespace mln::morpho
{
  ...
  mln::image2d<float> gaussian2d(InputImage input, float h_sigma, float v_sigma);

  template <class InputImage>
  void gaussian2d(InputImage input, float h_sigma, float v_sigma, mln::image2d<float>& out);
  ...
  namespace details
  {
    ...
    public:
      ...
      void execute(mln::box2d roi, float h_sigma, float v_sigma);
  ...
  */
  ...
  void gaussian2d(InputImage input, float h_sigma, float v_sigma, mln::image2d<float>& out)
  {
    static_assert(std::is_convertible_v<mln::image_value_t<InputImage>, float>);

    mln_entering("mln::morpho::gaussian2d");


    details::TileLoader<InputImage, float>          l_in(input, false);
    details::TileLoader<mln::image2d<float>, float> l_out(out, false);
    details::TileWriter<mln::image2d<float>, float> w(out, false);
