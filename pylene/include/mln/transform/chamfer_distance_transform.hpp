...
namespace mln
{
  ...
  namespace transforms
  {
    ...
    void chamfer_distance_transform(const Image<I>& f, const Neighborhood<N>& nbh, OutputImage&& out,
                                    bool background_is_object = false, SumOp add = SumOp());

    template <typename DistanceType = int, class I, class N>
    mln_ch_value(I, DistanceType)
        chamfer_distance_transform(const Image<I>& f, const Neighborhood<N>& nbh, bool background_is_object = false);
    ...
    void chamfer_distance_transform(const Image<I>& f_, const Neighborhood<N>& nbh_, OutputImage&& out,
                                    bool background_is_object, SumOp add)

    {
      static_assert(std::is_convertible<mln_value(I), bool>::value, "Input value type must be convertible to bool");
