...
namespace mln::morpho::details
{
  ...
  void vgaussian2d_algo_t::execute(mln::box2d roi, float h_sigma, float v_sigma)
  {
    ...
    float* tile_data     = (float*)std::malloc(tile_height * kBlockWidth * sizeof(float)); // FIXME: think about line memory alignment
    float* aux_data      = (float*)std::malloc(tile_height * kBlockWidth * sizeof(float)); // FIXME: think about line memory alignment
    ...
    if (v_sigma != 0.f)
    {
      ...
      for (int x = 0; x < width; x += kBlockWidth)
      {
        ...
        mln::box2d region(x0 + x, y0, w, y0 + height);
