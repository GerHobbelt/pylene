...
using P = mln::ndpoint<2, short>;

TEST(Core, Algorithm_Sort_UInt8)
{
  const mln::image2d<std::uint8_t> ima = {{1, 4, 2}, {3, 6, 8}};

  std::vector<P> points = mln::sort_points(ima);
...
TEST(Core, Algorithm_Sort_Decreasing_UInt8)
{
  ...
  std::vector<P> points = mln::sort_points(ima, std::greater<std::uint8_t>());
  std::vector<P> ref = {{0, 0}, {2, 0}, {0, 1}, {1, 0}, {1, 1}, {2, 1}};
...
TEST(Core, Algorithm_Sort_RGB8)
{
  ...
  std::vector<P> points = mln::sort_points(ima, mln::lexicographicalorder_less<mln::rgb8>());
